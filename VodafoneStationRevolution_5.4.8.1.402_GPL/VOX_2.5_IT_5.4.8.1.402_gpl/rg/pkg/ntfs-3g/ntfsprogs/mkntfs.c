...
static s64 ntfs_rlwrite(struct ntfs_device *dev, const runlist *rl,
		...
		WRITE_TYPE write_type, int logfile)
{
	...
	for (i = 0; rl[i].length; i++) {
		...
		do {
			...
			default :
				if (logfile)
				{
					/* Write contents of buffer numerous
					* times instead of using a large buffer which
					* causes a memory error */
					bytes_written = dev->d_ops->write(dev, val,
						MIN(logfile, length));
				}
				else
	...
	if (flags & ATTR_COMPRESSION_MASK) {
		...
		if ((type == AT_DATA)
			...
			bw = ntfs_rlwrite(g_vol->dev, rl, val, val_len,
					&inited_size, WRITE_LOGFILE, logfile);
		else
			bw = ntfs_rlwrite(g_vol->dev, rl, val, val_len,
					&inited_size, WRITE_STANDARD, logfile);
		if (bw != val_len) {
...
static int add_attr_data_positioned(MFT_RECORD *m, const char *name,
		...
		const u8 *val, const s64 val_len, int logfile)
{
	int err;

	err = insert_positioned_attr_in_mft_record(m, AT_DATA, name, name_len,
			ic, flags, rl, val, val_len, logfile);
	if (err < 0)
		...
		const runlist *rl, const u8 *bitmap, const u32 bitmap_len)
{
	...
	err = insert_positioned_attr_in_mft_record(m, AT_BITMAP, name, name_len,
			ic, const_cpu_to_le16(0), rl, bitmap, bitmap_len, 0);
	if (err < 0)
...
static BOOL mkntfs_create_root_structures(void)
{
	...
	err = add_attr_data_positioned(m, NULL, 0, CASE_SENSITIVE,
			const_cpu_to_le16(0), g_rl_mft, g_buf, g_mft_size, 0);
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
	...
	err = add_attr_data_positioned(m, NULL, 0, CASE_SENSITIVE,
			...
			g_rl_mftmirr[0].length * g_vol->cluster_size, 0);
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
	...
	err = add_attr_data_positioned(m, NULL, 0, CASE_SENSITIVE,
			...
			(const u8*)NULL, g_logfile_size, 0);
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
	...
	err = add_attr_data_positioned(m, NULL, 0, CASE_SENSITIVE,
			const_cpu_to_le16(0), g_rl_boot, (u8*)bs, 8192, 0);
	if (!err)
		err = create_hardlink(g_index_block, root_ref, m,
	...
	err = add_attr_data_positioned(m, "$Bad", 4, CASE_SENSITIVE,
			...
			g_vol->nr_clusters * g_vol->cluster_size, 0);
	if (!err) {
		err = add_attr_data(m, NULL, 0, CASE_SENSITIVE,
				const_cpu_to_le16(0), NULL, 0);
	}
...
static int mkntfs_redirect(struct mkntfs_options *opts2)
{
	...
	if (a->non_resident) {
		...
		lw = ntfs_rlwrite(g_vol->dev, rl, (const u8*)NULL,
			 g_lcn_bitmap_byte_size, NULL, WRITE_BITMAP, 0);
		err = errno;
		free(rl);

...
static unsigned char oobbuf[64];
...
int main (int argc, char *argv[])
{
	...
	erase_info_t erase;
	int isNAND, bbtest = 1;
	unsigned char *oobpoi = (unsigned char *)&cleanmarker;
	...
	if (jffs2) {
		...
		if (!isNAND)
			...
			if (oobinfo.useecc == MTD_NANDECC_AUTOPLACE) {
				int i, off, bytes, len;
				unsigned char *clnmrk_p = (unsigned char *)&cleanmarker;

				memset(oobbuf, 0xff, sizeof(oobbuf));
				oobpoi = oobbuf;

				len = meminfo.oobsize==8 ? 2 : 8;

				for (i = 0; oobinfo.oobfree[i][1] && len; i++) {
					off = oobinfo.oobfree[i][0];
					bytes = oobinfo.oobfree[i][1];
					if (bytes > len)
					    bytes = len;
					memcpy(oobpoi + off, clnmrk_p, bytes);
					clnmrk_p += bytes;
					len -= bytes;
				}

...
static int ipc_connect_ip_ex(u16 port, u32 addr, int quiet_conn_fail)
{
	 ...
	 * Therefore, a RG_IPC_SYNC_STR command is used to make sure the connect
	 * succeeded.
...
int ipc_client_sync(int fd)
{
    ...
    if (ipc_string_write(fd, RG_IPC_SYNC_STR))
	goto Exit;
    if (!(sync = ipc_string_read(fd)) || strcmp(sync, RG_IPC_SYNC_STR))
	goto Exit;
    rc = 0;

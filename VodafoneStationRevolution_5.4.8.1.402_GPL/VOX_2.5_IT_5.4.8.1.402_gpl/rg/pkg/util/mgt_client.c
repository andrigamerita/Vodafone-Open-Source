...
{
    char cmd[REBOOT_REASON_SIZE + 64];
    if (reason)
    {
        snprintf(cmd, sizeof(cmd), "crash set_reboot_reason %d %s:%s():%d",
	    (int)reason, reason_file, reason_func, reason_line);
        mgt_command(cmd, NULL);
    }
}
    ...
    char *reason_file, const char *reason_func, int reason_line)
{
    ...
    mgt_set_reboot_reason_command(reason, reason_file, reason_func,
        reason_line);

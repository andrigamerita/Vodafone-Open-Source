...
int (*tunnel_new_request_cb)(l2tp_tunnel *, uint32_t, uint32_t, uint16_t,
    uint32_t);
...
/**********************************************************************
* %FUNCTION: tunnel_delete_inactive
* %ARGUMENTS:
*  None
* %RETURNS:
*  Nothing
* %DESCRIPTION:
*  Free tunnels which are in IDLE state and received only one SCCRQ
*  Should be called only if L2TPD received stop notification
***********************************************************************/
void tunnel_delete_inactive(void)
{
    void *cursor;
    l2tp_tunnel *tunnel;
	...
	tunnel = hash_next(&tunnels_by_my_id, &cursor))
    {
	if (tunnel->state == TUNNEL_IDLE && tunnel->Nr == 1)
	    tunnel_free(tunnel);
    }
}
		    ...
		    struct sockaddr_in *from)
{
	...
	tunnel->peer_addr.sin_addr.s_addr, ntohs(tunnel->peer_addr.sin_port),
	l2tp_get_ip_by_fd(tunnel->sock)))
    {
	goto Error;
    }
    return;

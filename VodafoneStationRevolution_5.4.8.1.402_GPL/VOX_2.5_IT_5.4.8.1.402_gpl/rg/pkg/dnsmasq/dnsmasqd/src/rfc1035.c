			...
			for (addrlist = intr->addr; addrlist; addrlist = addrlist->next)
			  if (((addrlist->flags & ADDRLIST_IPV6) ? T_AAAA : T_A) == type)
				...
				continue;
			      ...
			      if (!is_local && strcmp(src_if_name, intr->intr))
				  continue;
	      ...
	      if ((crecp = cache_find_by_name(NULL, name, now, flag | (dryrun ? F_NO_RR : 0))))
		{
		  ...
		  if ((crecp->flags & (F_HOSTS | F_DHCP | F_CONFIG)) ||
			...
			if (crecp->flags & F_NEG)
			  ...
			  {
			    /* Hostnames provided via config (--host-record)
			     * are forwarded for local clients */
			    if (crecp->uid == SRC_CONFIG && (is_local || (!option_bool(OPT_REVERSE_PROXY) && daemon->guest_dev_name
				&& strcmp(src_if_name, daemon->guest_dev_name))))
			      break;

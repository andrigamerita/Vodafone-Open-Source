...
static void check_dns_listeners(time_t now, int drop);
static void sig_handler(int sig);
static void async_event(int pipe, time_t now);
static void fatal_event(struct event_desc *ev, char *msg);
static int read_event(int fd, struct event_desc *evp, char **msg);
static void poll_resolv(int force, int do_reload, time_t now);
static unsigned get_forwarders_count(struct server *list);
...
int main (int argc, char **argv)
{
  ...
  daemon->fwdrs_count = get_forwarders_count(daemon->servers);
...
static void async_event(int pipe, time_t now)
{
  pid_t p;
  struct event_desc ev;
  int i, check = 0, full_clearing_needed = 0;
  char *msg;
  ...
  if (read_event(pipe, &ev, &msg))
	...
	/* In case of servers_file or resolv_files specified there is a chance
	 * that DNS servers were changed and cache entries became invalid.
	 * So, we need to clear the cache entirely.*/
	full_clearing_needed = daemon->servers_file || (daemon->resolv_files && option_bool(OPT_NO_POLL));
	
	/* fall through */
	
      case EVENT_INIT:
	clear_cache_and_reload(now, ev.event == EVENT_INIT || full_clearing_needed);
...
void clear_cache_and_reload(time_t now, int clear_resolved_entries)
{
  ...
  if (daemon->port != 0)
    cache_reload(clear_resolved_entries);
...
static void check_dns_listeners(time_t now, int drop)
{
  ...
  for (listener = daemon->listeners; listener; listener = listener->next)
	...
	receive_query(listener, now, drop); 
	  ...
	  */
	  ...
	  if (!client_ok)
		...
		fcntl(confd, F_SETFL, flags & ~O_NONBLOCK);
	      ...
	      buff = tcp_request(listener, confd, now, &tcp_addr, netmask, auth_dns, listener->iface->name);

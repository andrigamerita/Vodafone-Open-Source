...
#define OPT_PRIMARY        62
#define OPT_BLOCK_LOCAL    63
#define OPT_REVERSE_PROXY  64
#define OPT_LAST           65
...
struct frec {
  ...
  size_t header_len;
  struct dns_header *edns_header;
  size_t edns_header_len;
#ifdef HAVE_DNSSEC 
...
extern struct daemon {
  ...
  /* DNS fail handling  */
  int fail_limit;
  int query_timeout_fail_counter;
  int fail_period;
  int fail_divisor;
  time_t queue_change_timeout;
  int default_queue_len;

  char *guest_dev_name;
  struct addr_list *blocked_hosts;
  unsigned fwdrs_count;

  unsigned short add_mac_opt;
...
void cache_reload(int clear_resolved_entries);
...
size_t answer_request(struct dns_header *header, char *limit, size_t qlen,  
		      ...
		      time_t now, int ad_reqd, int do_bit, int have_pseudoheader, int is_local,
		      char *src_if_name);
...
int random_sock(int family, char *intname);
...
void clear_cache_and_reload(time_t now, int clear_resolved_entries);
...
int insert_mac(union mysockaddr *addr, unsigned char *mac, int maclen);

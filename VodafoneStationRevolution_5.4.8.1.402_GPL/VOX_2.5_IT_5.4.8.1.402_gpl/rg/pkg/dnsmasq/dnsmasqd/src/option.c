...
#define LOPT_FAIL_LIMIT    362
#define LOPT_FAIL_DIVISOR  363
#define LOPT_FAIL_PERIOD   364
#define LOPT_PRIMARY       365
#define LOPT_BLOCK_LOCAL   366
#define LOPT_REVERSE_PROXY 367
#define LOPT_GUEST_DEV     368
#define LOPT_BLOCKED_HOSTS 369
#define LOPT_ADD_MAC_OPT   370
#define LOPT_FORCE_NDISC   371
...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "fail-limit", 1, 0, LOPT_FAIL_LIMIT },
    { "fail-period", 1, 0, LOPT_FAIL_PERIOD },
    { "fail-divisor", 1, 0, LOPT_FAIL_DIVISOR },
    { "primary-server", 0, 0, LOPT_PRIMARY },
    { "block-local", 0, 0, LOPT_BLOCK_LOCAL },
    { "reverse-proxy", 0, 0, LOPT_REVERSE_PROXY },
    { "guest-dev", 1, 0, LOPT_GUEST_DEV },
    { "blocked-hosts", 1, 0, LOPT_BLOCKED_HOSTS},
    { "add-mac-opt", 1, 0, LOPT_ADD_MAC_OPT },
    { "force-ndisc", 0, 0, LOPT_FORCE_NDISC },
    { NULL, 0, 0, 0 }
  };
...
static struct {
  ...
  { LOPT_FAIL_LIMIT, ARG_ONE, "<limit>", gettext_noop("After limit of consecutive queries timeouts reduce dns-forward-max to dns-forward-max/divisor times."), NULL },
  { LOPT_FAIL_PERIOD, ARG_ONE, "<period>", gettext_noop("Reduce dns-forward-max each period if servers are not responding."), NULL },
  { LOPT_FAIL_DIVISOR, ARG_ONE, "<divisor>", gettext_noop("Divide dns-forward-max on divisor to reduce queue length."), NULL },
  { LOPT_PRIMARY, OPT_PRIMARY, NULL, gettext_noop("Send DNS queries to all forwarders while primary is unavailable. Then only to it."), NULL },
  { LOPT_BLOCK_LOCAL, OPT_BLOCK_LOCAL, NULL, gettext_noop("Block local requests (from lo interface)"), NULL },
  { LOPT_REVERSE_PROXY, OPT_REVERSE_PROXY, NULL, gettext_noop("Indicate reverse proxy is turned on"), NULL },
  { LOPT_GUEST_DEV, ARG_DUP, "<iface>", gettext_noop("Set guest interface name"), NULL },
  { LOPT_BLOCKED_HOSTS, ARG_DUP, "[=<ipaddr>]...", gettext_noop("Listed hosts wouldn't be served. SERVFAIL in response"), NULL },
  { LOPT_ADD_MAC_OPT, ARG_ONE, "<unsigned short>", gettext_noop("Performs non starndard option number for add-mac feauture"), NULL },
  { LOPT_FORCE_NDISC, ARG_ONE, NULL, gettext_noop("Performs synchronous hardware address lookup"), NULL },
  { 0, 0, NULL, NULL, NULL }
}; 
...
static int primary_server_exists(struct server *list)
{
...
static void server_list_enqueue(struct server **list, struct server *server)
{
    struct server **last;
...
#  endif
      ...
      else
	{
      ...
      /* set default option */
      daemon->add_mac_opt = EDNS0_OPTION_MAC;
      break;
    ...
    case LOPT_GUEST_DEV: /* --guest-dev */
      if (arg)
	daemon->guest_dev_name = opt_string_alloc(arg);
      else
	ret_err(gen_err);
      break;

    case LOPT_BLOCKED_HOSTS: /* --blocked-hosts */
      while (arg)
	{
	  struct addr_list *host = opt_malloc(sizeof(struct addr_list));
	  comma = split(arg);
	  if (!(inet_pton(AF_INET, arg, &host->addr) > 0))
	    ret_err(_("Bad blocked host address"));
	  host->next = daemon->blocked_hosts;
	  daemon->blocked_hosts = host;
	  arg = comma;
	}
      break;
	...
	serv = newlist;
	if (option == 'S' && !primary_server_exists(daemon->servers))
	    serv->is_primary = 1;
	...
	server_list_enqueue(&daemon->servers, serv);
	break;
      }
    ...
    case LOPT_FAIL_LIMIT: /* --fail-limit */
      if (!atoi_check(arg, &daemon->fail_limit))
	ret_err(gen_err);
      if (!daemon->fail_limit)
        daemon->fail_limit = FORWARD_FAIL_LIMIT;
      break;

    case LOPT_FAIL_PERIOD: /* --period */
      if (!atoi_check(arg, &daemon->fail_period))
	ret_err(gen_err);
	break;

    case LOPT_FAIL_DIVISOR: /* --divisor */
      if (!atoi_check(arg, &daemon->fail_divisor))
	ret_err(gen_err);
      if (!daemon->fail_divisor)
        daemon->fail_divisor = FORWARD_FAIL_DIVISOR;
      break;

    case '0':  /* --dns-forward-max */
      if (!atoi_check(arg, &daemon->ftabsize))
	ret_err(gen_err);
      daemon->default_queue_len = daemon->ftabsize;
      break;  
...
#endif
    case LOPT_ADD_MAC_OPT:
      {
	  int opt;
	  if (!atoi_check16(arg, &opt))
	      ret_err(gen_err);
	  daemon->add_mac_opt = opt;
	  break;
      }
    case LOPT_FORCE_NDISC:
      daemon->force_ndisc = 1;
      break;
    default:
...
void read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  daemon->default_queue_len = FTABSIZ;
  daemon->fail_limit = FORWARD_FAIL_LIMIT;
  daemon->fail_divisor = FORWARD_FAIL_DIVISOR;

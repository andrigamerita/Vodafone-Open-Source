...
static void free_frec(struct frec *f, unsigned int flags);

/* is_response_err() checks the response received. 
 * Returns 1 if response is an error
 */
static int is_response_err(int rcode)
{
  switch (rcode)
    {
      case NOERROR:
...
static int is_host_blocked(struct in_addr host)
{
  struct addr_list *it;
  for (it = daemon->blocked_hosts; it; it = it->next)
    if (it->addr.s_addr == host.s_addr)
      return 1;
  return 0;
}
...
static struct server *get_next_forwarder(struct server *sentto)
{
  struct server *server;

  if (!(server = sentto->next))
    server = daemon->servers; /* at end of list, recycle */
			 ...
			 struct frec *forward, int ad_reqd, int do_bit)
{
  ...
  struct server *start = NULL, *invalid_server = NULL;
  ...
  if (forward || (forward = lookup_frec_by_sender(ntohs(header->id), udpaddr, hash)))
      ...
      if (forward->blocking_query)
	{
      ...
      if (option_bool(OPT_ORDER))
        start = get_next_forwarder(forward->sentto);
      else if (difftime(now, forward->time) >= TIMEOUT)
	{
      ...
      if (forward)
	{
	  ...
	  forward->retry = 0;
	  if ((forward->header = malloc(plen)))
	  {
	    memcpy(forward->header, header, plen);
	    forward->header_len = plen;
	  }
	  if (norebind)
  ...
  if (!flags && forward)
      ...
      if (find_pseudoheader(header, plen, &edns0_len, &pheader, NULL, NULL))
	{
      ...
      if (forward->flags & FREC_HAS_EXTRADATA)
	{
	  if ((forward->edns_header = malloc(plen)))
	    {
	      memcpy(forward->edns_header, header, plen);
	      forward->edns_header_len = plen;
	    }
	}
	...
	{ 
	  ...
	  if (type == (start->flags & SERV_TYPE) &&
	      ...
	      else 
		{
		  if (start->addr.sa.sa_family == AF_INET6)
		    {
		      if (!forward->rfd6 &&
			  !(forward->rfd6 = allocate_rfd(AF_INET6, start->interface)))
			break;
		    ...
		    {
		      if (!forward->rfd4 &&
			  !(forward->rfd4 = allocate_rfd(AF_INET, start->interface)))
			break;
	  ...
	  start = get_next_forwarder(start);

	  if (forward->retry && (start == invalid_server))
	    start = get_next_forwarder(start);
	...
	return 1;
      ...
      free_frec(forward, F_SERVFAIL); /* cancel */
    }	  
...
void reply_query(int fd, int family, time_t now)
{
  ...
  if (is_response_err(RCODE(header)) &&
      forward->forwardall == 0 &&
      daemon->fwdrs_count > 1 &&
      forward->retry == 0)
	...
	for (server = forward->sentto->next; server; server = server->next)
      ...
      if (!is_sign && server)
	{
	  /* in case ends0 use stored header to retry */
	  if (forward->flags & FREC_HAS_EXTRADATA)
	    {
	      forward->retry = 1;
	      forward_query(-1, NULL, NULL, 0, forward->edns_header, forward->edns_header_len,
			    now, forward, forward->flags & FREC_AD_QUESTION, forward->flags & FREC_DO_QUESTION);
	  ...
	  if ((nn = resize_packet(header, (size_t)n, pheader, plen)))
		...
		add_do_bit(header, nn,  (unsigned char *)pheader + plen);
	      if (daemon->last_server == forward->sentto)
		daemon->last_server = NULL;
	      forward->retry = 1;
  ...
  if ((forward->sentto->flags & SERV_TYPE) == 0)
	...
	if (!option_bool(OPT_ALL_SERVERS) && (!option_bool(OPT_PRIMARY) || (option_bool(OPT_PRIMARY) && is_primary)))
	  {
  ...
  if (forward->forwardall == 0 || --forward->forwardall == 1 ||
		  ...
		  if (--orig->work_counter == 0 || !(new = get_new_frec(now, NULL, orig)))
		      ...
		      else
			{
			  ...
			  if (server->addr.sa.sa_family == AF_INET6)
			    {
			      if (new->rfd6 || (new->rfd6 = allocate_rfd(AF_INET6, start->interface)))
				fd = new->rfd6->fd;
			    }
			  else
			    {
			      if (new->rfd4 || (new->rfd4 = allocate_rfd(AF_INET, start->interface)))
				fd = new->rfd4->fd;
			    }
			}
			      ...
			      forward->flags & FREC_ADDED_PHEADER, forward->flags & FREC_HAS_SUBNET, &forward->frec_src.source)))
	{
...
#endif
	  if (daemon->fail_period && daemon->query_timeout_fail_counter)
	    {
	      daemon->query_timeout_fail_counter = 0;
	      daemon->queue_change_timeout = 0;
	      daemon->ftabsize = daemon->default_queue_len;
	      my_syslog(LOG_WARNING, _("Restore query queue length to default: %d"), daemon->default_queue_len);
	    }
      ...
      free_frec(forward, 0); /* cancel */
    }
}


void receive_query(struct listener *listen, time_t now, int drop)
{
  ...
  if (option_bool(OPT_LOCAL_SERVICE))
      ...
      if (family == AF_INET6) 
	{
	  for (addr = daemon->interface_addrs; addr; addr = addr->next)
	    if ((addr->flags & ADDRLIST_IPV6) && addr->if_index == listen->iface->index &&
  ...
  if (daemon->force_ndisc)
  {
      unsigned char hwaddr[DHCP_CHADDR_MAX];
      int nl_fd = ndisc_event_open();
      int hwlen;

      if (!find_mac(&source_addr, hwaddr, 0, now))
      {
	  if ((hwlen = ndisc_resolve(nl_fd, listen->iface->index, &source_addr, hwaddr)))
	      insert_mac(&source_addr, hwaddr, hwlen);
	  else
	  {
	      ndisc_event_close(nl_fd);
	      return;
	  }
      }
      ndisc_event_close(nl_fd);
  ...
  if (auth_dns)
	...
	ad_reqd = 1;
      ...
      if (!strcmp(listen->iface->name, IF_LOOPBACK_NAME))
	is_local = 1;

      if (family == AF_INET && is_host_blocked(source_addr.in.sin_addr) ||
	  option_bool(OPT_BLOCK_LOCAL) && is_local)
        {
	  if (!(m = answer_request(header, ((char *) header) + udp_size, (size_t)n, 
				   dst_addr_4, netmask, now, ad_reqd, do_bit, have_pseudoheader, is_local,
				   listen->iface->name)))
	    {
	      m = setup_reply(header, n, NULL, F_SERVFAIL, 0);
	    }
...
unsigned char *tcp_request(struct listener *listen, int confd, time_t now,
			   union mysockaddr *local_addr, struct in_addr netmask, int auth_dns, char *src_if_name)
{
  size_t size = 0;
  int norebind = 0;
#ifdef HAVE_AUTH
  ...
  while (1)
	...
	{ 
	  ...
	  if (!find_mac(&peer_addr, hwaddr, 0, now))
	  {
	      if ((hwlen = ndisc_resolve(nl_fd, listen->iface->index, &peer_addr, hwaddr)))
		  insert_mac(&peer_addr, hwaddr, hwlen);
	      else
	      {
		  ndisc_event_close(nl_fd);
		  return packet;
...
#endif
	{
	   ...
	   m = answer_request(header, ((char *) header) + 65536, (size_t)size, 
			      dst_addr_4, netmask, now, ad_reqd, do_bit, have_pseudoheader, 0, src_if_name);
...
struct randfd *allocate_rfd(int family, char *intname)
{
  static int finger = 0;
  int i;
  ...
  for (i = 0; i < RANDOM_SOCKS; i++)
    if (daemon->randomsocks[i].refcount == 0)
      {
	if ((daemon->randomsocks[i].fd = random_sock(family, intname)) == -1)
	  break;
...
static void free_frec(struct frec *f, unsigned int flags)
{
  ...
  if (flags && f->header)
    {
      f->header_len = setup_reply(f->header, f->header_len, NULL, flags, 0);
      send_from(f->frec_src.fd, option_bool(OPT_NOWILD) || option_bool(OPT_CLEVERBIND), (char*)f->header, f->header_len,
		&f->frec_src.source, &f->frec_src.dest, f->frec_src.iface);
      free(f->header);
      f->header = NULL;
      f->header_len = 0;
    }
...
static int queue_len_reduce(time_t now)
{
  struct frec *f, *tmp;

  daemon->query_timeout_fail_counter++;
  if (daemon->fail_period && daemon->query_timeout_fail_counter >= daemon->fail_limit &&
      difftime(now, daemon->queue_change_timeout) >= daemon->fail_period)
    {
      /* free all frecs in queue */
      f = daemon->frec_list;
      while (f)
      {
	free_frec(f, F_SERVFAIL);
	tmp = f;
	f = f->next;
	free(tmp);
      }
      ...
      daemon->ftabsize /= daemon->fail_divisor;
      daemon->queue_change_timeout = now;
      if (daemon->ftabsize < 2)
	daemon->ftabsize = 2;

      my_syslog(LOG_WARNING, _("Reduce query queue length to: %d"), daemon->ftabsize);
      return 1;
    }
    ...
    if (!f->sentto)
    ...
    else 
      {
...
#endif
	      {
	        if (difftime(now, f->time) >= TIMEOUT && !f->expired)
		  {
		    f->expired = 1;
		    daemon->last_server = NULL;
		  }
		...
		if (difftime(now, f->time) >= 4*TIMEOUT)
		  {
		    free_frec(f, F_SERVFAIL);
		    if (queue_len_reduce(now))
		      {
			target = allocate_frec(now);
			break;
		      }
		    else
			target = f;

...
char *police_action_n2a(int action, int dscp, char *buf, int len)
{
	switch (action) {
	case -1:
	        if (dscp == -1)
		    return "continue";
		snprintf(buf, len, "continue(dscp 0x%x)", dscp);
		return buf;
		break;
	case TC_POLICE_OK:
		return "pass";
...
print_police(struct action_util *a, FILE *f, struct rtattr *arg)
{
	...
	action = tb[TCA_POLICE_RESULT] ? *(int*)RTA_DATA(tb[TCA_POLICE_RESULT])
	    : -1;
	fprintf(f, "action %s", police_action_n2a(action, p->under_dscpval,
	    b1, sizeof(b1)));
	fprintf(f, "/%s", police_action_n2a(p->action, p->over_dscpval, b1,
	    sizeof(b1)));
	fprintf(f, "\nref %d bind %d\n",p->refcnt, p->bindcnt);

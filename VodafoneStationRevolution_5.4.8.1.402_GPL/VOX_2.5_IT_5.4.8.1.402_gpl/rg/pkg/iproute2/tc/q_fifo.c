...
static int pfifo_fast_print_xstats(struct qdisc_util *qu, FILE *f, 
    struct rtattr *xstats)
{
	...
	if (RTA_PAYLOAD(xstats) != sizeof(*st)*3)
		return -1;
	st = RTA_DATA(xstats);
	for (i = 0; i < 3; i++)
	{
	    fprintf(f, " band %d- Sent %llu bytes %u pkts "
		"(dropped %u, overlimits %u qlen %d)\n", i,
		st[i].bytes, st[i].packets, st[i].drops, st[i].overlimits,
		st[i].qlen);
	}
...
struct qdisc_util pfifo_fast_qdisc_util = {
	...
	.print_xstats = pfifo_fast_print_xstats,
};

...
#ifndef JMK_ROOT
#error JMK_ROOT must be define to rg_include dir
#endif

#ifndef JMKE_BUILDDIR
#error JMKE_BUILDDIR must be define to rg_include dir
#endif
...
void add_libc_includes(char **gcc_argv, int *i, int use_ulibc, int use_stdinc)
{
	...
	if (use_stdinc)
	{
	    gcc_argv[(*i)++] = "-I" JMKE_BUILDDIR "/pkg/build/ulibc/include";
	    gcc_argv[(*i)++] = "-I" JMKE_BUILDDIR "/" ULIBC_DIR "/include";
	}
	else
	{
	    /* The include path must lead to libc_config_int.h */
	    gcc_argv[(*i)++] = "-I" JMKE_BUILDDIR "/pkg/build/ulibc/include/libc_rg";
	}
    }
#endif
    ...
    {
	if (use_stdinc)
	    gcc_argv[(*i)++] = "-I" JMKE_BUILDDIR "/pkg/build/glibc/include";
	else
	{
	    /* The include path must lead to libc_config_int.h */
	    gcc_argv[(*i)++] = "-I" JMKE_BUILDDIR "/pkg/build/glibc/include/libc_rg";
	}
    }
#endif
}
 ...
 */
	...
	if (a[0] == '-')
	{
	     ...
	     * There may be one small difference for some platforms, which
	     * specify crtbeginT.o as start file for static linkage. however, no
	     * meaningfull difference observed between that and default
	     * crtbegin.o */
	...
	gcc_argv[i++] = "-Wl,-rpath-link," JMKE_BUILDDIR "/pkg/build/lib";
#endif

	if (use_start)
	{
	    if (!shared)
		gcc_argv[i++] = JMKE_BUILDDIR "/pkg/build/lib/crt1.o";
	    gcc_argv[i++] = JMKE_BUILDDIR "/pkg/build/lib/crti.o";
#ifdef CONFIG_HAS_MMU
	    if (shared)
		gcc_argv[i++] = GCC_PREFIX "/crtbeginS.o";
	    else
#endif
	...
	if (use_stdlib)	
	{
	    gcc_argv[i++] = "-L" JMKE_BUILDDIR "/pkg/build/lib/";
	    gcc_argv[i++] = "-Wl,--start-group,-lgcc,-lgcc_eh,-lc,--end-group";
#ifdef CONFIG_AEABI
	    gcc_argv[i++] = "-lc_so";
#endif
	}
...
#ifdef CONFIG_HAS_MMU
	    if (shared)
		gcc_argv[i++] = GCC_PREFIX "/crtendS.o";
	    else
#endif
		gcc_argv[i++] = GCC_PREFIX "/crtend.o";
	    gcc_argv[i++] = JMKE_BUILDDIR "/pkg/build/lib/crtn.o";
	}
    }    
...
#ifdef GCC_HAS_INCLUDE_FIXED
	gcc_argv[i++] = "-I" GCC_PREFIX "/include-fixed/";
#endif
	gcc_argv[i++] = "-I" JMK_ROOT "/pkg/include/";
	gcc_argv[i++] = "-I" JMKE_BUILDDIR "/pkg/include/";
    }

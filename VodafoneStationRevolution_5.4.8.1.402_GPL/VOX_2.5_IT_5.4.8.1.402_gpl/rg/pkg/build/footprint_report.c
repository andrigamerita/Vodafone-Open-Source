...
static int is_mainfs_in_flash = cCONFIG_RG_MAINFS_IN_FLASH;
...
typedef struct {
    ...
    int sections;
    char filetype; 
    int is_in_disk_image;
} mount_t;

mount_t mount_list[] = {
    { MOUNT_CRAMFS, "cramfs", "mainfs_dir", "mainfs.img", ~SECTION_OTHER,
        TYPE_REGULAR_FILE, 1 },
    { MOUNT_MODFS, "modfs", "modfs_dir", "mod.img", ~SECTION_OTHER,
        TYPE_REGULAR_FILE, 1 },
    { MOUNT_RAMDISK, "ramdisk", "ramdisk_dir", "rd.img", SECTION_ALL,
	TYPE_REGULAR_FILE, 1 },
    { MOUNT_LINUX, "linux", LINUX_DIR_ABS "vmlinux", LINUX_DIR_ABS "vmlinux",
	SECTION_OTHER, TYPE_REGULAR_FILE, 0 },
...
static void usage(void)
{
	...
	"  --mainfs_dir mainfs_dir\n"
	"  --modfs_dir modfs_dir\n"
	"  --ramdisk_dir ramdisk_dir\n"
	"  --mainfs_img mainfs.img\n"
	"  --modfs_img mod.img\n"
	"  --linux_img ../../os/linux/vmlinux\n"
	"  --archiver archiver commandline (default is gzip -f9 -c)\n"
	"  --mainfs_in_flash (default set if CONFIG_RG_MAINFS_IN_FLASH "
	" is enabled)\n"
    ...
    mount_t *mount, int *section, struct mem_t *mem)
{
	...
	*section = SECTION_FILE;
    ...
    if (is_mainfs_in_flash && !strcmp(mount->name, "cramfs"))
    {
	ram = 0;
	slab = 0;
    }
...
void get_object_section_list(char ***sections, char* object_name)
{
	...
	if (!str_re_strs(*line, "^[ ]*\\[[ 0-9]*\\] (\\.[^ ]*) ",NULL,
	    &section, NULL) &&
	    (!strstr(object_name, LINUX_DIR_ABS "vmlinux") ||
    ...
    file_footprint **main_file_list, file_footprint **all_file_list)
{
	...
	for (f = files; *f; f++)
	{
	    ...
	    if (mount->filetype != TYPE_SYMLINK)
	    {
		if (!strstr(filename, LINUX_DIR_ABS "vmlinux"))
		{
		    ...
		    filename = NULL;
		    str_printf(&filename, "%s/%s", builddir, filesrcpath);
		}
	    }
		...
		if (mount->mount == MOUNT_LINUX)
		{
			...
			"linux_static_mod", NULL, NULL, 0, TYPE_SYMLINK};

		    str_printf(&path, "%s/"LINUX_DIR_ABS"modules_static",
			builddir);
		    static_modules.dir = path;

		    /* no need to add static modules to main_file_list beacuse
		     * they are not counted when summing mounts and sections
		     */
		    static_modules_total =
...
int is_mount_valid(mount_t *mount)
{
    ...
    /* validate mount dir */
    str_printf(&dir, "%s/%s", mount->is_in_disk_image ? disk_image : builddir,
	mount->dir);
    mount->dir = dir;
    if (!file_exist(mount->dir))
    {
	rg_error(LWARN, "Missing directory (%s) for mount (%s): skipped\n",
...
int main(int argc, char *argv[])
{
    ...
    char *list_file_name = NULL, *disk_image_file = NULL;
    mount_t *mount;
    char *s = NULL, *s2 = NULL;
	...
	else if (!strcmp(*argv, "--mainfs_in_flash"))
	{
	    is_mainfs_in_flash = 1;
	}
	else if (!strcmp(*argv, "--builddir"))
	{
	    argv++;	    
	    builddir = *argv;
	}
    ...
    if (!disk_image)
	str_printf(&disk_image, "%s/pkg/build/disk_image", builddir);
    ...
    str_printf(&disk_image_file, "%s/pkg/build/disk_image_file.txt", builddir);
    if (!(disk_img_fp = fopen(disk_image_file, "r")))
	rg_error(LEXIT, "failed opening disk_image_file.txt");

    fprintf(report_fp, "MAINFS_IN_FLASH: %s\n\n", is_mainfs_in_flash? "y" :
	"n");
    /* arguments are valid */
	...
	{
	    detect_and_add_files_to_lists(mount, &main_file_list,
		&all_file_list);

...
detect_gcc_features()
{
    detect_app CC "/usr/local/openrg/i686-jungo-linux-gnu/bin/i686-jungo-linux-gnu-gcc cc gcc" 1
    detect_cc_version
...
output_results()
{
	...
	echo 'JMK_LDLIBS:=-lnsl -lresolv -lsocket $(JMK_LDLIBS)' >> $om
    fi
	...
	echo 'JMK_LDLIBS:=-liconv $(JMK_LDLIBS)' >> $om
    fi
}

...
# 1.for each .so found at  mainfs_dir/libthat lack the               #
# suitable _pic.a  look for the suitable .a and link it              #
# under build/debug/*_pic.a                                          #
#                                                                    #
# 2. run mklibs                                                      #
#                                                                    #
# 3.copy mklibs so back to mainfs_dir/lib                            #          #                                                                    #
#                                                                    #
######################################################################
use strict;
use POSIX;
...
my $mainfs_lib_dir = qq{$build/pkg/build/disk_image/mainfs_dir/lib/};
 ...
 foreach my $link (<$ramdisk_lib_dir/*>)
  {
    ...
    next if (-e qq{$build/pkg/build/disk_image/mainfs_dir/lib/$basel});
       die qq{$build/pkg/build/disk_image/mainfs_dir/lib/$baset doesnt exists\n}
    if (! -e qq{$build/pkg/build/disk_image/mainfs_dir/lib/$baset});
    copy qq{$build/pkg/build/disk_image/mainfs_dir/lib/$baset},qq{$mklib_root_dir/lib/$basel};
...
sub run_mklibs
{
  ...
  my $command = qq{
python $rg_root/pkg/build/mklibs -D -v --target $prefix --ldlib ${prefix}-ld   -L $build/pkg/build/disk_image/mainfs_dir/lib  -L $build/debug -L $mklib_root_dir/lib -d $mklib_root_dir/dist $build/pkg/build/disk_image/mainfs_dir/bin/* 2>&1

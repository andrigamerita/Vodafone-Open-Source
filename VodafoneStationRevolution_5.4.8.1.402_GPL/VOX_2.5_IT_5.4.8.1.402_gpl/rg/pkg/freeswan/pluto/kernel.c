...
bool do_command(struct connection *c, const char *verb)
{
    ...
    else if (!strcmp(verb, "retry_timeout"))
	cmd = IPSEC_CMD_RETRY_TIMEOUT;
    else
	return TRUE;
...
setup_half_ipsec_sa(struct state *st, bool inbound)
{
	...
	*said_next = said;
	...
	if (st->st_esp.attrs.transid == ESP_AES)
                        ...
                        }
			{
			    ...
			    idle_time = idle_time_int;
                        ...
                        if(idle_time >= idle_max) {
                               DBG(DBG_KLIPS,
                                DBG_log("SA %s found idle for at least %ld sec",
                                        text_said, idle_max));
                                ret = TRUE;
                        }
                        else {

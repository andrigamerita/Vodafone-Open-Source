			      ...
			      uint32_t eroute_pid)
{
	...
	hold_eroute.er_emask.sen_proto = iph->protocol ? ~0:0;
	...
	if (ipsec_breakroute(&(hold_eroute.er_eaddr), &(hold_eroute.er_emask),
		...
		KLIPS_PRINT(debug_pfkey,
			    "klips_debug:ipsec_tunnel_start_xmit: "
			    "HOLD breakroute deleted " NIPQUAD_FMT ":%u -> "
			    NIPQUAD_FMT ":%u %u\n",
...
typedef struct ipsec_xmit_ctx_t {
    struct sk_buff *skb;
...
static void __ipsec_tunnel_start_xmit(ipsec_xmit_ctx_t *ctx, int status)
{
    	struct sk_buff *skb = ctx->skb;
	struct net_device *dev = ctx->dev;
	int after_encrypt = ctx->after_encrypt;
	struct eroute *er = ctx->er;
	__u8 *saved_header = ctx->saved_header;
	struct ipsec_sa *tdbp = ctx->tdbp;
	int hard_header_len = ctx->hard_header_len;
	unsigned orig_len = ctx->orig_len;
	struct ipsec_sa *stats_tdbp = ctx->stats_tdbp;
	...
	struct ipsec_sa *tdbq;		/* Tunnel Descriptor Block pointers */
	char sa[SATOA_BUF];
	size_t sa_len;
...
#ifdef CONFIG_IPSEC_NAT_TRAVERSAL
	uint8_t natt_type = ctx->natt_type;
	uint8_t natt_head = ctx->natt_head;
	uint16_t natt_sport = ctx->natt_sport;
	uint16_t natt_dport = ctx->natt_dport;
#endif
	int tdb_unlock = 0;
	...
	if(iph->protocol == IPPROTO_UDP) {
		if(skb->sk) {
                        ...
                        sport = ntohs(inet_sk(skb->sk)->inet_sport);
                        dport = ntohs(inet_sk(skb->sk)->inet_dport);
#else
			struct udp_sock *us;
...
encapsulate_loop:
	...
	do {
		...
		*/
		...
		stats_tdbp = tdbp = ipsec_sa_getbyid(&outgoing_said);		
		...
		if ((tdbp->ips_natt_type) && (!natt_type)) {
			...
			tdbp = tdbp->tdb_onext;
#ifdef CONFIG_IPSEC_ALG
			/* Full packet processing (including ESP or AH headers) needs
			 * room at the end of original packet with enough space
			 * for ESP/AH encapsulation.
			 */
			if (ixt_j && ixt_j->ixt_j_is_full_process_orig_pkt)
			        tailroom += headroom;
...
inner_encapsulate:
		...
		while (tdbp) {
			...
			if (ixt_j && ixt_j->ixt_j_is_full_process_orig_pkt)
			{
			        dat = skb_push(skb, is_ipip_exist ? sizeof(struct iphdr) : 0);
			}
			else
			...
			{
			        if (is_ipip_exist)
			                memmove((void *)dat, (void *)(dat + sizeof(struct iphdr)), iphlen);
			}
			else
			...
			case IPPROTO_ESP:
...
#ifdef CONFIG_IPSEC_ALG
				if (!ixt_e && !after_encrypt)

#endif /* CONFIG_IPSEC_ALG */
				switch(tdbp->tdb_encalg) {
#if defined(USE_SINGLE_DES) || defined(CONFIG_IPSEC_ENC_3DES) || defined(CONFIG_IPSEC_ENC_AES)
#ifdef USE_SINGLE_DES
				case ESP_DES:
#endif /* USE_SINGLE_DES */
#ifdef CONFIG_IPSEC_ENC_3DES
...
#endif
				{
				        ...
				        /* Generate IV randomly per each packet */
				...
				if (ixt_e || ixt_j) {
					if (!ixt_j) {
					        int ret, i;
				                ...
				                /* Generate IV randomly per each packet */
				                for (i = 0; i < ixt_e->ixt_ivlen/8; i++)
						{
						        iv[i] = (jiffies % 0xff) + i;
						}
						memcpy(espp->esp_iv, iv, ixt_e->ixt_ivlen/8);
			...
			if (ixt_j)
			{
				...
				complete_cb = __ipsec_tunnel_start_xmit;
				if (!after_encrypt)
				{
					ctx = kmalloc(sizeof(ipsec_xmit_ctx_t),
					    GFP_ATOMIC);
					...
					ctx->after_encrypt = 1;
					ctx->er = er;
					ctx->saved_header = saved_header;
					ctx->hard_header_len = hard_header_len;
					ctx->tdbp = tdbp; 
					ctx->natt_type = natt_type;
					ctx->natt_head = natt_head;
					ctx->natt_dport = natt_dport;
					ctx->natt_sport = natt_sport;
					ctx->orig_len = orig_len;
					ctx->stats_tdbp = stats_tdbp;
#endif

					status =
					    ipsec_alg_joint_auth_and_encrypt(
						ctx ? ctx->skb : NULL, tdbp,
						(caddr_t)espp, len - iphlen,
						headroom, authlen, espp->esp_iv,
						IPSEC_ALG_ENCRYPT, complete_cb,
						ctx);
 ...
 bypass:
	...
	struct flowi fl;
 	...
 	fl.flowi_oif = physdev->iflink;
 	fl.nl_u.ip4_u.daddr = ip_hdr(skb)->daddr;
 	fl.nl_u.ip4_u.saddr = pass ? 0 : ip_hdr(skb)->saddr;
 	fl.flowi_tos = RT_TOS(ip_hdr(skb)->tos);
 	fl.flowi_proto = ip_hdr(skb)->protocol;
...
#endif
#else
	error = ip_route_output(&rt,
				    ip_hdr(skb)->daddr,
				    pass ? 0 : ip_hdr(skb)->saddr,
				    RT_TOS(ip_hdr(skb)->tos),
				    physdev->iflink /* rgb: should this be 0?
						     */);
#endif
	if (rt)
	    rt_dst = &ipsec_route_dst(rt);
	...
	if(skb->len < skb_network_header(skb) - skb->data) {
		ipsec_log(KERN_WARNING
		       ...
		       skb_network_header(skb) - skb->data, skb->len);
		goto tx_errors;
	}
	__skb_pull(skb, skb_network_header(skb) - skb->data);
	...
	KLIPS_IP_PRINT(debug_tunnel & DB_TN_XMIT, ip_hdr(skb));
#ifdef NETDEV_23	/* 2.4 kernels */
	{
		int err;

		err = NF_HOOK(PF_INET, LSW_NF_INET_LOCAL_OUT, skb, NULL,
		    rt_dst->dev, ipsec_tunnel_xmit2);
	...
	while (stats_tdbp)
	{
            stats_tdbp->ips_life.orig_bytes += orig_len;
	    stats_tdbp->ips_life.orig_packets++;
	    stats_tdbp = stats_tdbp->ips_onext;
	}
...
{
    ipsec_xmit_ctx_t *ctx;

    ctx = kmalloc(sizeof(ipsec_xmit_ctx_t), GFP_ATOMIC);
    if (!ctx)
    {
#if defined(HAS_NETIF_QUEUE) || defined (HAVE_NETIF_QUEUE)
	netif_wake_queue(dev);
    ...
    memset(ctx, 0, sizeof(ipsec_xmit_ctx_t));
    ctx->skb = skb;
    ctx->dev = dev;
    ctx->orig_len = skb->len - (unsigned)(skb_network_header(skb) - skb->data);
    __ipsec_tunnel_start_xmit(ctx, 0);
    return 0;
}
...
#endif /* NET_21 */
{
	...
	/* Physical dev must implement rebuild_header */
...
DEBUG_NO_STATIC void
ipsec_tunnel_cache_update(struct hh_cache *hh, const struct net_device *dev, const unsigned char *  haddr)
{
	struct ipsecpriv *prv = netdev_priv(dev);
...
ipsec_tunnel_attach(struct net_device *dev, struct net_device *physdev)
{
	...
	prv->dev = physdev;
	prv->hard_start_xmit = dev_ops_start_xmit(physdev);
	prv->get_stats = dev_ops_get_stats(physdev);

	if (dev_header_ops_create(physdev))
		prv->hard_header = dev_header_ops_create(physdev);
	
	if (dev_header_ops_rebuild(physdev))
		prv->rebuild_header = dev_header_ops_rebuild(physdev);
	
	if (dev_ops_set_mac_address(physdev))
		prv->set_mac_address = dev_ops_set_mac_address(physdev);
	...
	if (dev_header_ops_cache_update(physdev))
		prv->header_cache_update = dev_header_ops_cache_update(physdev);
...
ipsec_device_event(struct notifier_block *unused, unsigned long event, void *ptr)
{
	...
	case NETDEV_UNREGISTER:
		...
		for(cur=ipsec_dev_head; cur; cur=cur->next) {
			...
			if(ipsec_dev) {
				priv = (struct ipsecpriv *)
					netdev_priv(ipsec_dev);
				if(priv) {
					;
...
#ifdef HAVE_NETDEV_HEADER_OPS
	/* we don't use header ops, ipsecX is an IP device only.
...
int add_ipsec_dev(char *name)
{
	...
	*cur = (ipsec_dev_list *)kmalloc(sizeof(ipsec_dev_list), GFP_KERNEL);
	if (!*cur)
		return -1;
	(*cur)->ipsec_dev = alloc_netdev(sizeof(struct ipsecpriv), name,
		ipsec_tunnel_netdev_setup);
	if (!(*cur)->ipsec_dev)
		goto Error;
	(*cur)->next = NULL;
#ifdef USE_NETDEV_OPS
	(*cur)->ipsec_dev->netdev_ops = &klips_device_ops;
#else
...
EXPORT_SYMBOL(add_ipsec_dev);

int del_ipsec_dev(char *name)
{
    ipsec_dev_list **cur = &ipsec_dev_head;
...
EXPORT_SYMBOL(del_ipsec_dev);

int ipsec_dev_anti_replay_set(char *name)
{
    pfkey_anti_replay_enabled_set(strcmp(name, "off"));
    return 0;
}
EXPORT_SYMBOL(ipsec_dev_anti_replay_set);

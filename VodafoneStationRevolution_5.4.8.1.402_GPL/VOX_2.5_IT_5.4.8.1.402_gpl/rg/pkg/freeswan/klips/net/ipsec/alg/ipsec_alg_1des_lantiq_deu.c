 ...
 * 
 *  rg/pkg/freeswan/klips/net/ipsec/alg/ipsec_alg_1des_lantiq_deu.c * 
 * 
...
/* deu driver functions */
extern int ifx_deu_des_setkey(void *ctx, const u8 *key, unsigned int keylen);
extern void ifx_deu_des_cbc(void *ctx, uint8_t *dst, const uint8_t *src,
    uint8_t *iv, size_t nbytes, int encdec, int inplace);

static int _1des_deu_set_key(struct ipsec_alg_enc *alg, __u8 *key_e,
    const __u8 *key, size_t keysize)
{
    struct des_ctx *ctx = (struct des_ctx *)key_e;

    DEBUG_PRINT("1des_deu_set_key(alg=%p, key_e=%p, key=%p keysize=%d)\n",
	alg, key_e, key, keysize);

    return ifx_deu_des_setkey(ctx, key, keysize);
}

static int _1des_deu_cbc_encrypt(struct ipsec_alg_enc *alg, __u8 *key_e,
    __u8 *in, int ilen, const __u8 *iv, int encrypt)
{
    struct des_ctx *ctx = (struct des_ctx *)key_e;
    __u8 iv_buf[ESP_DES_CBC_BLKLEN] = {};

    DEBUG_PRINT("1des_deu_cbc_encrypt(alg=%p, key_e=%p, in=%p ilen=%d "
...
static struct ipsec_alg_enc ipsec_alg_1DES_deu = {
    ...
    ixt_e_set_key:	_1des_deu_set_key,
    ixt_e_cbc_encrypt:	_1des_deu_cbc_encrypt,
};
...
IPSEC_ALG_MODULE_INIT(ipsec_1des_init)
{
    ...
    if (esp_id)
	ipsec_alg_1DES_deu.ixt_alg_id = esp_id;
    if (excl)
	ipsec_alg_1DES_deu.ixt_state |= IPSEC_ALG_ST_EXCL;
    ret = register_ipsec_alg_enc(&ipsec_alg_1DES_deu);
    if (debug)
    {
	ipsec_log("ipsec_1des_init(alg_type=%d alg_id=%d name=%s): ret=%d\n",
	    ipsec_alg_1DES_deu.ixt_alg_type,
	    ipsec_alg_1DES_deu.ixt_alg_id,
	    ipsec_alg_1DES_deu.ixt_name,
	    ret);
    }
    if (ret==0 && test)
    {
...
{
    unregister_ipsec_alg_enc(&ipsec_alg_1DES_deu);
}

        ...
        case IX_CRYPTO_ACC_STATUS_SUCCESS:
            ...
            else
            {
                ...
                if (pXmitDesc)
                {
                    ...
                    prv = netdev_priv(pXmitDesc->dev);
                    stats = (struct net_device_stats *) &(prv->mystats);

                    if (stats)
                        stats->tx_errors++;
		    spin_lock(&tdb_lock);
		    while (pXmitDesc->stats_tdbp)
		    {
			pXmitDesc->stats_tdbp->ips_life.orig_errors++;
			pXmitDesc->stats_tdbp = pXmitDesc->stats_tdbp->ips_onext;
		    }
		    spin_unlock(&tdb_lock);
...
bypass_cb:
    ...
    while (pXmitDesc->stats_tdbp)
    {
	pXmitDesc->stats_tdbp->ips_life.orig_bytes += pXmitDesc->orig_len;
	pXmitDesc->stats_tdbp->ips_life.orig_packets++;
	pXmitDesc->stats_tdbp = pXmitDesc->stats_tdbp->ips_onext;
    }
    spin_unlock(&tdb_lock);
...
tx_dropped:
    ...
    while (pXmitDesc->stats_tdbp)
    {
	pXmitDesc->stats_tdbp->ips_life.orig_dropped++;
	pXmitDesc->stats_tdbp = pXmitDesc->stats_tdbp->ips_onext;
    }
    spin_unlock(&tdb_lock);
    goto cleanup_cb;
...
ipsec_tunnel_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	unsigned orig_len = skb->len - (unsigned)(skb->nh.raw - skb->data);
	struct ipsec_sa *stats_tdbp = NULL;
	...
	do {
		...
		stats_tdbp = tdbp = ipsec_sa_getbyid(&(pXmitDesc->outgoing_said));
        /* store current tdbp into xmit desc */
        pXmitDesc->tdbp = tdbp;
		sa_len = satoa((pXmitDesc->outgoing_said), 0, sa, SATOA_BUF);
...
/*IXP425 glue code : crypto perform */
		...
		pXmitDesc->stats_tdbp = stats_tdbp;
		pXmitDesc->orig_len = orig_len;
...
{
	struct ipsecpriv *prv = netdev_prive(dev);

    ...
    IxCryptoAccStatus status)
{
    ...
    switch (status)
    {
        case IX_CRYPTO_ACC_STATUS_SUCCESS:
		    ...
		    spin_lock(&tdb_lock);
		    while (pRcvDesc->stats_tdbp)
		    {
			pRcvDesc->stats_tdbp->ips_life.orig_dropped++;
			pRcvDesc->stats_tdbp = pRcvDesc->stats_tdbp->ips_inext;
		    }
...
/*		skb->h.ipiph=(struct iphdr *)skb->data; */
	...
	while (pRcvDesc->stats_tdbp)
        {
	    pRcvDesc->stats_tdbp->ips_life.orig_bytes += skb->len;
	    pRcvDesc->stats_tdbp->ips_life.orig_packets++;
	    pRcvDesc->stats_tdbp = pRcvDesc->stats_tdbp->ips_inext;
	}
        spin_unlock(&tdb_lock);
...
rx_errors:
	...
	spin_lock(&tdb_lock);
        while (pRcvDesc->stats_tdbp)
        {
	    pRcvDesc->stats_tdbp->ips_life.orig_errors++;
	    pRcvDesc->stats_tdbp = pRcvDesc->stats_tdbp->ips_inext;
	}
        spin_unlock(&tdb_lock);
        goto rcvleave_cb;
...
*/
	    ...
	    pRcvDesc->stats_tdbp = stats_tdbp;

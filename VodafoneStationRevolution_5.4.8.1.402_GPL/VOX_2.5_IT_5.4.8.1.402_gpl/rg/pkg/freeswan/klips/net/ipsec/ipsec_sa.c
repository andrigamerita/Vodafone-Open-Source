...
ipsec_sa_wipe(struct ipsec_sa *ips)
{
	...
	if(ips->ips_j_ctx) {
#ifdef CONFIG_IPSEC_ALG
		if (ips->ips_alg_joint && ips->ips_alg_joint->ixt_j_destroy_ctx) {
			ips->ips_alg_joint->ixt_j_destroy_ctx(ips->ips_j_ctx);
		} else {
#endif /* CONFIG_IPSEC_ALG */
		memset((caddr_t)(ips->ips_j_ctx), 0, ips->ips_j_ctx_size);
		kfree(ips->ips_j_ctx);
#ifdef CONFIG_IPSEC_ALG
		}
#endif /* CONFIG_IPSEC_ALG */

...
typedef void (*ipsec_alg_complete_cb_t)(void *, int);
struct ipsec_alg_joint {
	IPSEC_ALG_STRUCT_COMMON
	unsigned ixt_j_ctx_size;
	int ixt_j_is_support_ah_proto;
	int ixt_j_is_full_process_orig_pkt;
	int (*ixt_j_set_key)(__u8 ipsec_authalg, __u8 ipsec_encalg, __u8 *j_ctx,
		const __u8 *key_a, int keylen_a, const __u8 *key_e,
		int keylen_e, int dir_in);
	/* Authenticate and decrypt/encrypt in one step.
	 * Return value: 0 for successful operation, non-zero otherwise.
	 * data - pointer to begining of the start of the data to authenticate
	 * 	  and encrypt (i.e. to the ESP header).
	 * len - size of data (including authentication trailer even if not
	 * 	 supplied - it will be written by this function).
	 * enc_offset - size of header at the begining of data that should be
	 * 		authenticated but not encrypted.
	 * authlen - size of (trimmed) authentication header (usually 12 for
	 * 	     ipsec).
	 */
	int (*ixt_j_auth_and_encrypt)(struct sk_buff *skb,
		struct ipsec_alg_enc *ixt_e, __u8 *j_ctx, const __u8 *data,
		int len, int enc_offset, int authlen, const __u8 *iv,
		int encrypt, ipsec_alg_complete_cb_t complete_cb, void *cb_ctx);

	void (*ixt_j_destroy_ctx)(__u8 *j_ctx);
};
/*	
...
#define IPSEC_ALG_TYPE_JOINT	16
...
#define IPSEC_ALG_SA_ESP_JOINT(sa_p) ((sa_p)->ips_alg_joint)
/* 

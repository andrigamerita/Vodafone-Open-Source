	...
	while(sk && ((skb=skb_dequeue(&(sk->sk_receive_queue)))!=NULL)) {
		...
		if((debug_pfkey && sysctl_ipsec_debug_verbose) &&
			...
			printk(" sk:%p", skb->sk);
                        ...
                        printk(" stamp:%d.%d", skb->tstamp.tv.sec,
			    skb->tstamp.tv.nsec / 1000);
			...
			printk(" h:%p", skb_transport_header(skb));
			printk(" nh:%p", skb_network_header(skb));
			printk(" mac:%p", skb_mac_header(skb));
			printk(" dst:%p", skb_dst(skb));
			...
			printk(" tail:%p", skb_tail_pointer(skb));
			printk(" end:%p", skb_end_pointer(skb));
			if(sysctl_ipsec_debug_verbose) {
				unsigned char* i;
				printk(" data");
...
pfkey_upmsg(struct socket *sock, struct sadb_msg *pfkey_msg)
{
	...
	skb_set_transport_header(skb, ipsec_skb_offset(skb,
	    skb_put(skb, pfkey_msg->sadb_msg_len * IPSEC_PFKEYv2_ALIGN)));
...
DEBUG_NO_STATIC int
pfkey_create(struct net *net, struct socket *sock, int protocol, int kern)
#else
DEBUG_NO_STATIC int
		...
		void *dat)
{
	...
	*peof = 0;
	...
	if(!sysctl_ipsec_debug_verbose) {
#endif /* CONFIG_IPSEC_DEBUG */
	BUF_PRINT("    sock   pid   socket     next     prev e n p sndbf    Flags     Type St\n");
#ifdef CONFIG_IPSEC_DEBUG
	} else {
	BUF_PRINT("    sock   pid d    sleep   socket     next     prev e r z n p sndbf    stamp    Flags     Type St\n");
	}
#endif /* CONFIG_IPSEC_DEBUG */
	...
	sk_for_each(sk, node, &pfkey_sock_list) {
		...
		if(!sysctl_ipsec_debug_verbose) {
#endif /* CONFIG_IPSEC_DEBUG */
		BUF_PRINT("%8p %5d %8p %d %d %5d %08lX %8X %2X\n",
			     sk,
			     key_pid(sk),
			     sk->sk_socket,
			     sk->sk_err,
		...
		BUF_PRINT("%8p %5d %d %8p %8p %d %d %d %d %5d %d.%06d %08lX %8X %2X\n",
			     sk,
			     key_pid(sk),
...
#endif /* PROC_FS_2325 */
int
pfkey_supported_read_proc(char *buffer,
			  char **start,
			  off_t offset,
			  int count,
			  int *peof,
			  void *dat)
{
	...
	*peof = 0;
	...
	BUF_PRINT("satype exttype alg_id ivlen minbits maxbits\n");
...
#endif /* PROC_FS_2325 */
int
pfkey_registered_read_proc(char *buffer,
			   char **start,
			   off_t offset,
			   int count,
			   int *peof,
...
pfkey_init(void)
{
	...
	static struct supported supported_init_esp[] = {
		...
		{SADB_EXT_SUPPORTED_ENCRYPT, SADB_EALG_NULL, 0, 0, 0},
#endif /* CONFIG_IPSEC_ENC_3DES */
#ifdef CONFIG_IPSEC_ENC_1DES
		{SADB_EXT_SUPPORTED_ENCRYPT, SADB_EALG_DESCBC, 64, 56, 56},
#endif /* CONFIG_IPSEC_ENC_3DES */
#ifdef CONFIG_IPSEC_ENC_3DES
		{SADB_EXT_SUPPORTED_ENCRYPT, SADB_EALG_3DESCBC, 64, 168, 168},
#endif /* CONFIG_IPSEC_ENC_3DES */
#ifdef CONFIG_IPSEC_ENC_AES
		{SADB_EXT_SUPPORTED_ENCRYPT, SADB_EALG_AESCBC, 128, 128, 256},
#endif /* CONFIG_IPSEC_ENC_3DES */
	};
...
#    endif /* PROC_FS_21 */
#  else /* !PROC_FS_2325 */
	create_proc_read_entry("pf_key", S_IRUGO, PROC_NET,
	    pfkey_read_proc, NULL);
	create_proc_read_entry("pf_key_supported", S_IRUGO, PROC_NET,
	    pfkey_supported_read_proc, NULL);
	create_proc_read_entry("pf_key_registered", S_IRUGO, PROC_NET,
	    pfkey_registered_read_proc, NULL);
#  endif /* !PROC_FS_2325 */
#endif /* CONFIG_PROC_FS */

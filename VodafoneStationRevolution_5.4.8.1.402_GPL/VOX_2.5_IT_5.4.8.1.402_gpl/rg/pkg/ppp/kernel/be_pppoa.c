...
static void pppoatm_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	pppoatm_ops.input(pvcc->ppp_sc, skb);
	return;
    error:
	kfree_skb(skb);
...
{
	struct pppoatm_vcc *pvcc = atmvcc_to_pvcc(atmvcc);
	pvcc->ppp_sc = pppoatm_ops.be_attach(if_name, 0, 0, rg_pppoa_output,
            pvcc);
	if (pvcc->ppp_sc && pvcc->delayed_lcp)
	{
	    pppoatm_ops.input(pvcc->ppp_sc, pvcc->delayed_lcp);
	    pvcc->delayed_lcp = NULL;
	}
...
{
	struct pppoatm_vcc *pvcc = atmvcc_to_pvcc(atmvcc);
	pppoatm_ops.be_detach(pvcc->ppp_sc);
	pvcc->ppp_sc = NULL;
	return 0;
}
...
#endif
	memset(&pppoatm_ops, 0, sizeof(pppoatm_ops));
}

...
{  
	...
	if (!cli_qfileinfo(targetcli, fnum, 
	    ...
	    if (!cli_getattrE(targetcli, fnum, 
			  &attr, (SMB_OFF_T *)&b_size, NULL, NULL, NULL)) {
...
static int do_getrange(char *rname, SMB_BIG_UINT start, SMB_BIG_UINT len)
{  
	int handle = 0, fnum;
	char *data;
	struct timeval tp_start;
	...
	if (!cli_qfileinfo(targetcli, fnum, &attr, &fsize, NULL, NULL, NULL,
		NULL, NULL)) {
		SMB_BIG_UINT b_size = fsize;
		if (!cli_getattrE(targetcli, fnum, 
			&attr, (SMB_OFF_T *)&b_size, NULL, NULL, NULL)) {
			d_printf("getattrib: %s\n", cli_errstr(targetcli));
			return 1;
	...
	if (start > fsize) {
		d_printf("Starting position %.0f greater then file size %.0f\n",
			(double)start, (double)fsize);
		return 1;
	}

	read_size = MIN(io_bufsize, len);

	DEBUG(1,("getting file %s range from offset %.0f length %.0f "
	         "read_size %.0f file_size %.0f\n", rname, (double)start,
		 (double)len, (double)read_size, (double)fsize));
	...
	while (start + nread < size) {
		int n = cli_read(targetcli, fnum, data, nread + start,
			read_size);

		DEBUG(1,("getrange read %d bytes, nread was %0.f\n", n,	(double)nread));

		if (n <= 0)
			break;
 
		if (writefile(handle,data, n) != n) {
			d_printf("Error writing to stdout\n");
			rc = 1;
			break;
		}
...
static int cmd_getrange(void)
{
	...
	if (!next_token_nr(NULL,p,NULL,sizeof(rname)-strlen(rname))) {
		d_printf("getrange <remote name> <start> <len>\n");
		return 1;
	}
	clean_name(rname);
	...
	start = (SMB_BIG_UINT)strtol(buf, (char **)NULL, 10);
	...
	len = (SMB_BIG_UINT)strtol(buf, (char **)NULL, 10);

	return do_getrange(rname, start, len);
}
	...
	if (reput) {
		...
		if (fnum >= 0) {
			if (!cli_qfileinfo(targetcli, fnum, NULL, &start, NULL, NULL, NULL, NULL, NULL)) {
			    SMB_BIG_UINT b_start = start;
			    if (!cli_getattrE(targetcli, fnum, NULL, (SMB_OFF_T *)&b_start, NULL, NULL, NULL)) {
				d_printf("getattrib: %s\n",cli_errstr(cli));
				return 1;
			    } else
				start = b_start;
...
static struct
{
  ...
  {"getrange",cmd_getrange,"<remote name> <start> <len> get a portion of a file to stdout",{COMPL_REMOTE,COMPL_REMOTE}},
...
/* XXX Don't split command up according to ';' as this doesn't support files
 * containing ';'. We need to support files containing ';' but don't need to
 * support multiple commands in one command string. The spliting code has been
 * removed */

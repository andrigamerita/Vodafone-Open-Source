...
static const char *charset_name(charset_t ch)
{
	...
	if (ch == CH_UTF16LE) ret = talloc_strdup(tmp_talloc_ctx(), "UTF-16LE");
	else if (ch == CH_UTF16BE) ret = talloc_strdup(tmp_talloc_ctx(), "UTF-16BE");
	else if (ch == CH_UNIX) ret = lp_unix_charset();
	else if (ch == CH_DOS) ret = lp_dos_charset();
	else if (ch == CH_DISPLAY) ret = lp_display_charset();
	else if (ch == CH_UTF8) ret = talloc_strdup(tmp_talloc_ctx(), "UTF8");
	...
	if (ret && !strcmp(ret, "LOCALE")) {
		...
		talloc_free(ret);
		ret = ln ? talloc_strdup(tmp_talloc_ctx(), ln) : NULL;
	}
#endif

	if (!ret || !*ret) ret = talloc_strdup(tmp_talloc_ctx(), "ASCII");
	return ret;
}
...
void init_iconv(void)
{
	...
	const char *utf16le = charset_name(CH_UTF16LE);

	/* so that charset_name() works we need to get the UNIX<->UCS2 going
	   first */
	if (!conv_handles[CH_UNIX][CH_UTF16LE])
		conv_handles[CH_UNIX][CH_UTF16LE] = smb_iconv_open(utf16le, "ASCII");

	if (!conv_handles[CH_UTF16LE][CH_UNIX])
		conv_handles[CH_UTF16LE][CH_UNIX] = smb_iconv_open("ASCII", utf16le);
	talloc_free((void *)utf16le);
		...
		for (c2=0;c2<NUM_CHARSETS;c2++) {
			...
			if (conv_handles[c1][c2] == (smb_iconv_t)-1) {
				...
				if (c1 != CH_UTF16LE && c1 != CH_UTF16BE) {
					talloc_free((void *)n1);
					n1 = talloc_strdup(tmp_talloc_ctx(), "ASCII");
				}
				if (c2 != CH_UTF16LE && c2 != CH_UTF16BE) {
					talloc_free((void *)n2);
					n2 = talloc_strdup(tmp_talloc_ctx(), "ASCII");
				}
	...
	if(retval==(size_t)-1) {
		...
		switch(errno) {
			...
			case E2BIG:
				reason="No more room"; 
				if (!conv_silent) {
				        const char *from_name = charset_name(from);
					const char *to_name = charset_name(to);
					if (from == CH_UNIX) {
					...
					talloc_free((void *)from_name);
					talloc_free((void *)to_name);
				}
				break;
			case EILSEQ:

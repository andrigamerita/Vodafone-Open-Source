...
static int vfswrap_ftruncate(vfs_handle_struct *handle, files_struct *fsp, int fd, SMB_OFF_T len)
{
	...
	/* XXX: we ignore truncate failure. Trying to force it on FAT file
	 * system, on which it is not supported, may take too long and will 
	 * cause the SMB client to hang up the connection. */
	result = 0;
	goto done;
#if 0

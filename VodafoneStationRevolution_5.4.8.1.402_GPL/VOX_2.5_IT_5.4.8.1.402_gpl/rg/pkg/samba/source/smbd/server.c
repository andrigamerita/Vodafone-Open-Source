...
/* Taken from util/net_groupmap.c 
 * some changes were made to satisfy our needs. */
static int net_groupmap_cleanup(void)
{
	GROUP_MAP *map = NULL;
	int i;
	size_t entries;
	...
	if (!pdb_enum_group_mapping(NULL, SID_NAME_UNKNOWN, &map, &entries,
				    ENUM_ALL_MAPPED)) {
		DEBUG(0, ("Could not list group mappings\n"));
		return -1;
	}

	for (i=0; i<entries; i++) {

	        /* Unmap all builtin groups */
		if (map[i].sid_name_use == SID_NAME_WKN_GRP)
		{
		    map[i].gid = -1;
		    pdb_update_group_mapping_entry(&map[i]);
		    continue;
		}

		if (map[i].gid == -1)
			DEBUG(1, ("Group %s is not mapped\n", map[i].nt_name));

		DEBUG(1, ("Deleting mapping for NT Group %s, sid %s\n",
		    map[i].nt_name,
...
/* Traverse /etc/groups and add all entries into group mapping data base */
#include <paths.h>
static void rebuild_group_mapping(void)
{
    struct group *group;
    DOM_SID g_sid;
    time_t mod_time = file_modtime(_PATH_GROUP);
    static time_t last_mod_time;

    /* No need to rebuild if /etc/group was not modified. */
    if (mod_time && last_mod_time == mod_time)
	return;

    last_mod_time = mod_time;

    /* Remove all entries from data base.
     * Will initiate data base if not exists */
    net_groupmap_cleanup();

    /* Get global SAM SID */
    sid_copy(&g_sid, get_global_sam_sid());

    /* Traverse /etc/group and add all entries. */
    for (setgrent(); (group = getgrent());)
    {
	uint32 rid;
	GROUP_MAP map = {};
	fstring string_sid = "";

	/* If group name match a builtin group name - map it. */
	if (pdb_getgrnam(&map, group->gr_name) &&
	    map.sid_name_use == SID_NAME_WKN_GRP)
	{
	    map.gid = group->gr_gid;
	    pdb_update_group_mapping_entry(&map);
	    continue;
	}
	
	/* Create string representation for group  */
	rid = algorithmic_pdb_gid_to_group_rid(group->gr_gid);
	sid_append_rid(&g_sid, rid);
	sid_to_string(string_sid, &g_sid);

	/* Remove group rid from g_sid */
	sid_split_rid(&g_sid, &rid);

	/* Add to list */
	add_initial_entry(group->gr_gid, string_sid, SID_NAME_ALIAS,
	    group->gr_name, "Local Group");
    }
}
...
BOOL reload_services(BOOL test)
{
	...
	if (lp_loaded()) {
		pstring fname;
		char *cfg_file = lp_configfile();
		pstrcpy(fname,cfg_file);
		talloc_free(cfg_file);
	...
	/* Close connections that are no longer available after change */
	conn_close_changed();
	...
	/* Rebuild group mappings if needed */
	rebuild_group_mapping();
 ...
 int main(int argc,const char *argv[])
{
	...
	lp_do_parameter(GLOBAL_SECTION_SNUM, "syslog only", "Yes");

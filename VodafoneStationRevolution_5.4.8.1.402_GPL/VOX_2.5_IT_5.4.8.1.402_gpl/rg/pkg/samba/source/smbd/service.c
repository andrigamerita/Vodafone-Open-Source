	...
	*user = 0;
	...
	if (conn->nt_user_token != NULL) {
		...
		for (i=0; i<conn->nt_user_token->num_sids; i++) {
			...
			size_t ngroups = conn->ngroups; /* 64bit fixup */
...
/* ROOT Activities: */	
	...
	/* Block smb preexec to avoid problemaitc command executions */
...
void close_cnum(connection_struct *conn, uint16 vuid)
{
	...
	/* Block smb postexec to avoid problemaitc command executions */
#ifdef CONFIG_RG_ALLOW_SMB_EXEC

				 ...
				 int is_guest)
{
	...
	if (!NT_STATUS_IS_OK(nt_status) && !NT_STATUS_EQUAL(nt_status, NT_STATUS_MORE_PROCESSING_REQUIRED)) {
		...
		/* 
		 * this flag(SMB_SETUP_GUEST) was set in reply_spnego_*()
		 * function and then cleared here in set_message().
		 * I didn't find a real purpose of the set_message(,,,True) here
		 * so I decided to re-set the flag because it is important
		 * for successful connection in a map_to_guest scenario */
		if (is_guest && NT_STATUS_IS_OK(nt_status))
				 ...
				 BOOL *p_invalidate_vuid)
{
	...
	*p = 0;
	...
	response = spnego_gen_auth_response(&ap_rep_wrapped, ret, OID_KERBEROS5_OLD);
	reply_sesssetup_blob(conn, outbuf, response, ret, is_guest);
				 ...
				 BOOL wrap) 
{
	...
	ret = reply_sesssetup_blob(conn, outbuf, response, nt_status, is_guest);
	if (wrap) {
		data_blob_free(&response);
	}

...
static BOOL has_permission(const char **list, user_struct *vuser)
{
    return user_in_list(vuser->user.unix_name ,list);
}

/* Check if share permissions have changed */
static int share_permissions_changed(int snum, connection_struct *conn)
{
    user_struct *vuser = get_valid_user_struct(conn->vuid);
    ...
    if (!has_permission(lp_writelist(snum), vuser))
    {
	if (!has_permission(lp_valid_users(snum), vuser))
	    return 1;
	new_read_only = True;
    }

    new_admin_user = has_permission(lp_admin_users(snum), vuser);
	
    if (new_read_only != conn->read_only || new_admin_user != conn->admin_user)
	return 1;
...
/****************************************************************************
close all conn structures that their service got deleted or its path changed
****************************************************************************/
void conn_close_changed(void)
{
	...
	if (lp_deleted(snum) || strcmp(lp_pathname(snum), conn->connectpath)
	    || share_permissions_changed(snum, conn))
	{

 ...
 */
typedef struct {
        BOOL deleted; /* For marking service as deleted in reload services */
	BOOL valid;
	BOOL autoloaded;
	int usershare;
...
#ifdef CONFIG_RG_ALLOW_SMB_EXEC
...
*/
	...
	str_list_free(&Globals.szWinbindNssInfo);
...
FN_LOCAL_BOOL(lp_deleted, deleted)
...
static int add_a_service(const service *pservice, const char *name)
{
	...
	if (name) {
		...
		if (i >= 0) {
		        /* Service may be marked as deleted by lp_killunused
			 * during reload_serives.
			 * Here as we see it is found, deleted is unset. */
		        ServicePtrs[i]->deleted = 0;
             ...
             BOOL initialize_globals)
{
	...
	services = lp_auto_services();
	lp_add_auto_services(services);
...
char *service_path(char *name)
{
    int service_num = lp_servicenumber(name);

    if (service_num == GLOBAL_SECTION_SNUM)
	return NULL;
    return ServicePtrs[service_num]->szPath;
}

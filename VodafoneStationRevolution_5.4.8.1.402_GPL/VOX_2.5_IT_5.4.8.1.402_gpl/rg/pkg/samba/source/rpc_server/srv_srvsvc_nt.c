 ...
 ********************************************************************/
static int is_visible(pipes_struct *p, int snum)
{
#ifdef CONFIG_RG_SMB_RESTRICTED_SHARE_VISIBILITY
    user_struct *user;

    if (!lp_restrict_anonymous())
	return True;

    user = get_valid_user_struct(p->vuid);

    if (!user || user->guest)
	return False;

    return user_in_list(user->user.unix_name, lp_valid_users(snum));
#else
    return True;
#endif
}
	       ...
	       uint32 info_level, uint32 *resume_hnd, uint32 *total_entries, BOOL all_shares)
{
	...
	*resume_hnd = 0;
	...
	for (snum = 0; snum < num_services; snum++) {
		if (is_visible(p, snum) && lp_browseable(snum) && lp_snum_ok(snum) && (all_shares || !is_hidden_share(snum)) )
			num_entries++;
	}

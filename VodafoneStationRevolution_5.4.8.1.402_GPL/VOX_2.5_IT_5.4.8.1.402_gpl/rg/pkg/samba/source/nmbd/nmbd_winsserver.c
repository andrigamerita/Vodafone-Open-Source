...
static void wins_hook(const char *operation, struct name_record *namerec, int ttl)
{
	...
	/* We are only intersted in 0x00 which represents a workstation */
	if (namerec->name.name_type)
	    return;

	p = command;
	p += slprintf(p, sizeof(command)-1, "%s %s %s", 
		      cmd,
		      operation, 
		      namestr);

	for (i=0;i<1 && i<namerec->data.num_ips;i++) {

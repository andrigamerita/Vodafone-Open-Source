...
extern struct printif rg_printif;
...
ssize_t print_job_write(int snum, uint32 jobid, const char *buf, SMB_OFF_T pos, size_t size)
{
	...
	if (pjob->pid != sys_getpid())
		return -1;
#ifndef CONFIG_RG_LPD
	return_code = write_data_at_offset(pjob->fd, buf, size, pos);
#else
	return_code = bio_write(pjob->fd, (void *)buf, size);
...
#ifdef CONFIG_RG_LPD
	/* pjob.filename is used to embed samba's job id into lpd for
	 * later retrieval */
	slprintf(pjob.filename, sizeof(pjob.filename)-1, "%s%.6d.XXXXXX", 
	    PRINT_SPOOL_PREFIX, jobid);
	pjob.fd = lpd_client_new_job(pjob.queuename, user->conn->user, 
	    user->conn->client_address, jobname, pjob.filename);
#else
		       ...
		       print_status_struct *status)
{
	...
	*ppqueue = NULL;
	...
	/* convert file names to meaningful job names */
	for (i=0; i<count; i++)
	{
	    	int jobid = print_parse_jobid((*ppqueue)[i].fs_file);
		struct printjob *pjob;
		...
		if (!pjob)
		    	continue;
		DEBUG(2, ("converting %s -> %s\n", (*ppqueue)[i].fs_file,
		    	pjob->jobname));
		fstrcpy((*ppqueue)[i].fs_file, pjob->jobname);
	}

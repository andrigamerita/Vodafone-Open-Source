...
static char *ntdriver_path(const char *name)
{
    extern char *service_path(char *);
    static char buf[256];
    char *path = service_path("print$");
...
BOOL nt_printing_init(void)
{
	...
	tdb_drivers = tdb_open_log(ntdriver_path("ntdrivers.tdb"), 0, TDB_DEFAULT, O_RDWR|O_CREAT, 0600);
	if (!tdb_drivers) {
		DEBUG(0,("nt_printing_init: Failed to open nt drivers database %s (%s)\n",
			ntdriver_path("ntdrivers.tdb"), strerror(errno) ));
		return False;
	}
 
	if (tdb_printers)
		tdb_close(tdb_printers);
	tdb_printers = tdb_open_log(ntdriver_path("ntprinters.tdb"), 0, TDB_DEFAULT, O_RDWR|O_CREAT, 0600);
	if (!tdb_printers) {
		DEBUG(0,("nt_printing_init: Failed to open nt printers database %s (%s)\n",
			ntdriver_path("ntprinters.tdb"), strerror(errno) ));
		return False;
	}
 
	if (tdb_forms)
		tdb_close(tdb_forms);
	tdb_forms = tdb_open_log(ntdriver_path("ntforms.tdb"), 0, TDB_DEFAULT, O_RDWR|O_CREAT, 0600);
	if (!tdb_forms) {
		DEBUG(0,("nt_printing_init: Failed to open nt forms database %s (%s)\n",
			ntdriver_path("ntforms.tdb"), strerror(errno) ));
		return False;
	}
...
WERROR nt_printer_publish(Printer_entry *print_hnd, int snum, int action)
{
	...
	putenv(KRB5_ENV_CCNAME "=MEMORY:prtpub_cache");

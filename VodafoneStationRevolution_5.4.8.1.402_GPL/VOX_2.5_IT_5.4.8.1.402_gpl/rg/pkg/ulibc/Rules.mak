...
# Some nice JMK_CFLAGS to work with
JMK_CFLAGS := -include $(top_builddir)include/libc-symbols.h \
	$(CFLAGS_ENVIR) $(RG_CFLAGS) $(XWARNINGS) $(CPU_CFLAGS) $(SSP_CFLAGS) \
	-fno-builtin -nostdinc -I$(top_builddir)include -I.

ifneq ($(strip $(UCLIBC_EXTRA_CFLAGS)),"")
JMK_CFLAGS += $(subst ",, $(UCLIBC_EXTRA_CFLAGS))
endif
...
#JMK_CFLAGS += $(call check_gcc,-std=gnu99,)

JMK_LDFLAGS_NOSTRIP:=$(CPU_LDFLAGS-y) -shared --warn-common --warn-once -z combreloc
# binutils-2.16.1 warns about ignored sections, 2.16.91.0.3 and newer are ok
#JMK_LDFLAGS_NOSTRIP+=$(call check_ld,--gc-sections)

ifeq ($(UCLIBC_BUILD_RELRO),y)
JMK_LDFLAGS_NOSTRIP+=-z relro
endif

ifeq ($(UCLIBC_BUILD_NOW),y)
JMK_LDFLAGS_NOSTRIP+=-z now
endif

JMK_LDFLAGS:=$(JMK_LDFLAGS_NOSTRIP) -z defs
ifeq ($(DODEBUG),y)
#JMK_CFLAGS += -g3
JMK_CFLAGS += -O0 -g3
else
JMK_CFLAGS += $(OPTIMIZATION) $(XARCH_CFLAGS)
endif
ifeq ($(DOSTRIP),y)
JMK_LDFLAGS += -s
else
...
JMK_CFLAGS+=$(call check_gcc,--combine,)
endif
else
DOMULTI:=n
endif
...
JMK_CFLAGS += -I$(KERNEL_HEADERS)

# Sigh, some stupid versions of gcc can't seem to cope with '-iwithprefix include'
#JMK_CFLAGS+=-iwithprefix include
JMK_CFLAGS+=-isystem $(shell $(CC) -print-file-name=include)
ifeq ($(GCC_HAS_INCLUDE_FIXED),y)
JMK_CFLAGS+=-isystem $(shell $(CC) -print-file-name=include-fixed)
endif
...
LIBGCC_CFLAGS ?= $(JMK_CFLAGS) $(CPU_CFLAGS-y)

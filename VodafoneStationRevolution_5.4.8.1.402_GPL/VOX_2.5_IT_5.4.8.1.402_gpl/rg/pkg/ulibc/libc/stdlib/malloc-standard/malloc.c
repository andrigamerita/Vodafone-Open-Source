...
#include <sys/param.h>
char malloc_printf_buf[MALLOC_PRINTF_MAX_LINE + 1];
#endif
...
#ifdef __CONFIG_RG_DBG_ULIBC_MALLOC__
	jhead = malloc(nb - MALLOC_ALIGN_MASK - JMALLOC_OVERHEAD);
	jhead--;
	/* Unlink from list of allocated blocks, because a recursive call of
	 * malloc takes place. */
...
#ifdef __CONFIG_RG_DBG_ULIBC_MALLOC__
    jhead = retval;
    jhead->magic = JMALLOC_MAGIC;
    jhead->alloc_ra = alloc_ra;
    jhead->len = orig_bytes;

    jhead->next = av->alloc_head.next;
    jhead->next->prev = jhead;

    av->alloc_head.next = jhead;
    jhead->prev = &av->alloc_head;

    jhead++;
    retval = jhead;
#ifdef __CONFIG_RG_DBG_ULIBC_MEM_POISON__
    memset(retval, 0xbe, MIN(orig_bytes, __CONFIG_RG_DBG_ULIBC_MEM_POISON__));
#endif

    memcpy(retval + orig_bytes, JMALLOC_FOOT, sizeof(JMALLOC_FOOT));
#endif
    __MALLOC_UNLOCK;
...
#ifdef __CONFIG_RG_DBG_ULIBC_MALLOC__
/* most significant bit is part of address and should not be used */ 
#define REPORTED_MARK_BIT (0x1)
#define ALLOCATOR(head) ((head)->alloc_ra)
#define REPORTED(head) (((unsigned long)ALLOCATOR(head)) & REPORTED_MARK_BIT)
#define MARK_AS_REPORTED(head) \
    ALLOCATOR(head) = \
    (void *)(((unsigned long)ALLOCATOR(head)) | REPORTED_MARK_BIT)
#define UNMARK_AS_REPORTED(head) \
    ALLOCATOR(head) = \
    (void *)(((unsigned long)ALLOCATOR(head)) & ~REPORTED_MARK_BIT)
#define ALLOC_SZ(head) ((head)->len)
#define CHUNK_SZ(head) chunksize(mem2chunk(head))

void dump_alloc_mem(void)
{
    jmalloc_header_t alloc_head, *iter, *iter2;
    mstate av;
    int cnt, grp_cnt, i;
    void *alloc_ra;
    long alloc_summ, chunk_summ, grp_chunk_summ;
    mchunkptr p;
    mbinptr b;
    ...
    if (av->alloc_head.next != &av->alloc_head)
    {
	alloc_head.next = av->alloc_head.next;
	alloc_head.prev = av->alloc_head.prev;

	alloc_head.next->prev = &alloc_head;
	alloc_head.prev->next = &alloc_head;
    ...
    {
	/* Empty list */
	alloc_head.next = alloc_head.prev = &alloc_head;
    }

    malloc_printf("\nbegin of memory status report >>>>>>>>>\n");
    /* clear least significant bit, set by previous report (or set by default
     * in thumb mode) */
    for (iter = alloc_head.next; iter !=  &alloc_head; iter = iter->next)
	UNMARK_AS_REPORTED(iter);

    cnt = 0;
    alloc_summ = chunk_summ = 0;
	...
	if (REPORTED(iter))
	    continue;
	iter2 = iter;
	alloc_ra = ALLOCATOR(iter);
	grp_cnt = 0;
	grp_chunk_summ = 0;
	while (iter2 !=  &alloc_head)
	{
	    if (alloc_ra == ALLOCATOR(iter2) &&
		ALLOC_SZ(iter) == ALLOC_SZ(iter2))
	    {
		grp_cnt++;
		grp_chunk_summ += CHUNK_SZ(iter2);

		MARK_AS_REPORTED(iter2);
	    }

	    iter2 =  iter2->next;
	}

	malloc_printf("%08x %8d %8d [%8d]\n", alloc_ra, grp_cnt, ALLOC_SZ(iter),
	    grp_chunk_summ);

	cnt += grp_cnt;
	alloc_summ += grp_cnt * ALLOC_SZ(iter);
	chunk_summ += grp_chunk_summ;
    }

    malloc_printf("Reported  %8d blocks for \n%8ld bytes\n%8ld gross\n",
	cnt, alloc_summ, chunk_summ);
	...
	for (i = 0; i < NFASTBINS; i++)
	{
	    ...
	    {
		cnt++;
		chunk_summ += chunksize(p);
		p = p->fd;
	    }
	}

	malloc_printf("fastbins contain %d chunks overall \n%8ld bytes\n", cnt,
	    chunk_summ);
    }
    ...
    malloc_printf("bins contain %d chunks overall \n%8ld bytes\n", cnt,
	chunk_summ);

    malloc_printf("Memory statistics: \nmmapped_mem \t%8d\nsbrked_mem \t%8d\n"
	"max_sbrked_mem \t%8d\nmax_mmapped_mem \t%8d\nmax_total_mem \t%8d\n",
	av->mmapped_mem, av->sbrked_mem, av->max_sbrked_mem,
	av->max_mmapped_mem, av->max_total_mem);

#ifdef __CONFIG_RG_DBG_ULIBC_MEM_POISON__
    malloc_printf("Quarantine statistics: \nn_quarantine \t%8d\n"
	"quarantine_mem \t%8d\n", av->n_quarantine, av->quarantine_mem);
#endif
    malloc_printf("end of memory dump <<<<<<<<<\n");

    iter = av->alloc_head.next;

    av->alloc_head.next = alloc_head.next;
    av->alloc_head.next->prev = &av->alloc_head;

    iter->prev = alloc_head.prev;
    iter->prev->next = iter;

    __MALLOC_UNLOCK;
}

void mem_save_alloc_ra(void *__ptr, void *alloc_ra)
{
    jmalloc_header_t *jhead = __ptr;
    ...
    {
	malloc_printf("%p:mem_save_alloc_ra(%p, %p) called from %p "
	    "for incorrect memory\n", getpid(), __ptr, alloc_ra,
	    __builtin_return_address(0));

	/* Let's give to complete the printing.
	 * mem_save_alloc_ra() is a function for improvement of debugging
	 * process. Therefore in case of error we'll crash without respect to
	 * CONFIG_RG_DBG_ULIBC_MALLOC_CRASH */
	sleep(2);
	*((int *)0) = 0xfaceface;
    }
    jhead->alloc_ra = alloc_ra;

...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.14.4.1.8.21 $")
				...
				if (f) {
					if (f->frametype == AST_FRAME_CONTROL) {
						...
						case AST_CONTROL_RINGBACK_TONE:
							if (option_verbose > 2)
								ast_verbose( VERBOSE_PREFIX_3 "%s is playing ringback tone\n", o->chan->name);
							if (!(*sentringing) && !ast_test_flag(outgoing, OPT_MUSICBACK)) {
								ast_indicate(in, AST_CONTROL_RINGBACK_TONE);
								(*sentringing)++;
							}
							break;
...
static int dial_exec_full(struct ast_channel *chan, void *data, struct ast_flags *peerflags)
{
	...
	/* If caller is different from template_caller and call is not internal
	 * limit the call */
	if (!ast_strlen_zero(template_caller) &&
	    ...
	    !jdsp_is_internal_call(chan, cur))
	{
	...
	if (!jdsp_is_internal_call(chan, cur))
	...
	if (peer) {
		...
		if (!jdsp_is_internal_call(chan, peer->type))

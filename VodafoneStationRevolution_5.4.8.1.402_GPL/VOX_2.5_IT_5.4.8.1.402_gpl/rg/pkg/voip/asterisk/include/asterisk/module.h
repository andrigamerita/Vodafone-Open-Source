...
/*! 
 * \brief Reload only if config file changed
 *
 * This function performs a reload only if the config file was changed.
 *
 * \return The return value is not used.
 */
int reload_if_changed(void);		/* reload configs */
 ...
 * \param check_conf_file  1 => reload only if conf file changed
 *
 * This function reloads the specified module, or if no modules are specified,
 * it will reload all loaded modules.
 *
 * \note Modules are reloaded using their reload() or reload_if_changed()
 * functions, not unloading them and loading them again.
 *

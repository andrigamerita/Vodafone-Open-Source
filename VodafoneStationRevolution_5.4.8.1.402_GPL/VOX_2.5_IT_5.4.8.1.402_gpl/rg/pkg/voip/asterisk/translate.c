...
struct ast_frame *ast_translate(struct ast_trans_pvt *path, struct ast_frame *f, int consume)
{
	...
	if (!ast_tvzero(f->delivery)) {
		...
		path->nextin = ast_tvadd(path->nextin, ast_samp2tv(f->samples, f->subclass == AST_FORMAT_SLINEAR16 ? 16000 : 8000));
	}
	delivery = f->delivery;
	if (consume)
		ast_frfree(f);
		...
		if (p->next) 
			...
			if (!ast_tvzero(delivery)) {
				...
				/* Predict next outgoing timestamp from samples in this
				   frame. */
				path->nextout = ast_tvadd(path->nextout, ast_samp2tv( out->samples, out->subclass == AST_FORMAT_SLINEAR16 ? 16000 : 8000));
			} else {
...
static void calc_cost(struct ast_translator *t, int samples)
{
	...
	int cost;
	int out_rate = t->dstfmt == AST_FORMAT_SLINEAR16 ? 16000 : 8000;

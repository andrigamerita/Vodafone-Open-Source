...
ASTERISK_FILE_VERSION(__FILE__, "$Revision: 1.9.2.1.2.28 $")
	...
	while(cat) {
		if (strcasecmp(cat, "general")) {
			...
			if (!strcasecmp(cat, user)) {
				...
				ast_sockaddr_from_sin(&temp_addr, &s->sin);
				if (ha && !ast_apply_ha(ha, &temp_addr)) {
...
static diagnostics_result do_diagnostics_outcall(char *chan_type, char *exten,
	char *trunk_user, int timeout, int require_answer)
{
	int cause = 0;
	...
	chan = ast_request(chan_type, &formats, dn, &cause);
	if (chan) 
	{
		...
		if (!ast_call(chan, dn, 0)) 
		{
			...
			ast_log(LOG_NOTICE, "Unable to start diagnostic call channel %s/%s\n", chan_type, dn);
	} else 
...
static void *diag_outcall_completion_thread(void *data)
{
	...
	res = do_diagnostics_outcall(outcall_data->type, outcall_data->exten,
		outcall_data->trunk, outcall_data->timeout, outcall_data->require_answer);
...
static int action_diag_outcall_start(struct mansession *s, struct message *m)
{
	...
	ast_log(LOG_DEBUG, "Need to start diagnostic call from trunk %s to %s with"
		" timeout %d type %s (require answer %d)\n", trunk_name, exten, dial_timeout,
		type, require_answer);
	...
	strncpy(outcall_data->type, type, sizeof(outcall_data->type));

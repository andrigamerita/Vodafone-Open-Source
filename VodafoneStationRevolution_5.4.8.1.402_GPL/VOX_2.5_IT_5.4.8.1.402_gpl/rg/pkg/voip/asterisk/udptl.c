...
static inline int udptl_debug_test_addr(struct ast_sockaddr *addr)
{
	if (udptldebug == 0)
		return 0;
	if (!ast_sockaddr_isnull(&udptldebugaddr)) {
		if (((ast_sockaddr_port(&udptldebugaddr))
			&& (ast_sockaddr_port(&udptldebugaddr) != ast_sockaddr_port(addr)))
			|| (!ast_sockaddr_cmp_addr(&udptldebugaddr, addr)))
		return 0;
	}
	return 1;
}
...
struct ast_frame *ast_udptl_read(struct ast_udptl *udptl)
{
	...
	if (udptl->nat) {
		...
		if (ast_sockaddr_cmp(&udptl->them, &addr)) {
			...
			ast_log(LOG_DEBUG, "UDPTL NAT: Using address %s:%d\n", ast_sockaddr_to_str(iabuf, sizeof(iabuf), &udptl->them), ast_sockaddr_port(&udptl->them));
		}
	}

	if (udptl_debug_test_addr(&addr)) {
		ast_verbose("Got UDPTL packet from %s:%d (type %d, seq %d, len %d)\n",
			ast_sockaddr_to_str(iabuf, sizeof(iabuf), &addr), ast_sockaddr_port(&addr), 0, seqno, res);
	}
#if 0
...
struct ast_udptl *ast_udptl_new_with_bindaddr(struct sched_context *sched, struct io_context *io, int callbackmode, struct ast_sockaddr *addr)
{
	...
	if ((udptl->fd = socket(ast_sockaddr_is_ipv6(addr) ? AF_INET6 : AF_INET, SOCK_DGRAM, 0)) < 0) {
		free(udptl);
...
{
	struct ast_sockaddr ia;
	ast_sockaddr_setnull(&ia);
...
void ast_udptl_set_peer(struct ast_udptl *udptl, struct ast_sockaddr *them)
{
        ast_sockaddr_copy(&udptl->them, them); 
}

void ast_udptl_get_peer(struct ast_udptl *udptl, struct ast_sockaddr *them)
{
	ast_sockaddr_copy(them, &udptl->them);
}

void ast_udptl_get_us(struct ast_udptl *udptl, struct ast_sockaddr *us)
{
...
int ast_udptl_write(struct ast_udptl *s, struct ast_frame *f)
{
	...
	if (len > 0  && !ast_sockaddr_isnull(&s->them)) {
		if ((res = ast_sendto(s->fd, buf, len, 0, &s->them)) < 0)
			ast_log(LOG_NOTICE, "UDPTL Transmission error to %s:%d: %s\n", ast_sockaddr_to_str(iabuf, sizeof(iabuf), &s->them), ast_sockaddr_port(&s->them), strerror(errno));
#if 0
		...
		if (udptl_debug_test_addr(&s->them))
			ast_verbose("Sent UDPTL packet to %s:%d (type %d, seq %d, len %d)\n",
					ast_sockaddr_to_str(iabuf, sizeof(iabuf), &s->them),
					ast_sockaddr_port(&s->them), 0, s->seqno, len);
	}
	...
	for (;;) {
		...
		if (ast_sockaddr_cmp(&t1, &ac1)) {
			ast_log(LOG_DEBUG, "Oooh, '%s' changed end address to %s:%d\n", 
				c1->name, ast_sockaddr_to_str(iabuf, sizeof(iabuf), &t1), ast_sockaddr_port(&t1));
			ast_log(LOG_DEBUG, "Oooh, '%s' was %s:%d\n", 
				c1->name, ast_sockaddr_to_str(iabuf, sizeof(iabuf), &ac1), ast_sockaddr_port(&ac1));
			ast_sockaddr_copy(&ac1, &t1);
		}
		if (ast_sockaddr_cmp(&t0, &ac0)) {
			ast_log(LOG_DEBUG, "Oooh, '%s' changed end address to %s:%d\n", 
				c0->name, ast_sockaddr_to_str(iabuf, sizeof(iabuf), &t0), ast_sockaddr_port(&t0));
			ast_log(LOG_DEBUG, "Oooh, '%s' was %s:%d\n", 
				c0->name, ast_sockaddr_to_str(iabuf, sizeof(iabuf), &ac0), ast_sockaddr_port(&ac0));
...
static int udptl_do_debug_ip(int fd, int argc, char *argv[])
{
	...
	if (hp == NULL)
		return RESULT_SHOWUSAGE;
	udptldebugaddr.ss.ss_family = AF_INET;
	udptldebugaddr.len = sizeof(struct sockaddr_in);
	memcpy(&((struct sockaddr_in *)&udptldebugaddr.ss)->sin_addr, hp->h_addr, hp->h_length);
	ast_sockaddr_set_port(&udptldebugaddr, port);
	if (port == 0)
		ast_cli(fd, "UDPTL Debugging Enabled for IP: %s\n", ast_sockaddr_to_str(iabuf, sizeof(iabuf), &udptldebugaddr));
	else
		ast_cli(fd, "UDPTL Debugging Enabled for IP: %s:%d\n", ast_sockaddr_to_str(iabuf, sizeof(iabuf), &udptldebugaddr), port);
	udptldebug = 1;

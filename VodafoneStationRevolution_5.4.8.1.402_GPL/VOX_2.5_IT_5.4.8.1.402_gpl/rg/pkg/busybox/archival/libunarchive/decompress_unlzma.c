...
static ALWAYS_INLINE rc_t* rc_init(int fd) /*, int buffer_size) */
{
	...
	memset(rc, 0, sizeof(*rc) + RC_BUFFER_SIZE);
...
unpack_lzma_stream(int src_fd, int dst_fd)
{
	...
	if (bb_full_read(src_fd, &header, sizeof(header)) != sizeof(header)
	 || header.pos >= (9 * 5 * 5)
	) {
 ...
 one_byte1:
			...
			if (buffer_pos == header.dict_size) {
				...
				if (bb_full_write(dst_fd, buffer, header.dict_size) != (ssize_t)header.dict_size)
					goto bad;
			}
#else
			len = 1;
...
one_byte2:
	...
	{
		int total_written = 0; /* success */

		if (bb_full_write(dst_fd, buffer, buffer_pos) != (ssize_t)buffer_pos) {
 bad:

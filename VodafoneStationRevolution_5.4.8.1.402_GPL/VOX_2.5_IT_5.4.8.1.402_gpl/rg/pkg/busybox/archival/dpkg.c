...
enum dpkg_opt_e {
	...
	dpkg_opt_skip_same_version = 512
};
...
static void run_package_script_or_die(const char *package_name, const char *script_type, const char *fmt, ...)
{
	char *script_path;
	int result, len;
	va_list ap;

	script_path = xmalloc(strlen(package_name) + strlen(script_type) + 21 + 256);
	len = sprintf(script_path, "/var/lib/dpkg/info/%s.%s", package_name, script_type);
	...
	if (access(script_path, F_OK)) {
		...
		script_path[len++] = ' '; 
		va_start(ap, fmt);
		vsnprintf(script_path + len, 255, fmt, ap);
		va_end(ap);
		result = system(script_path);
	}
	free(script_path);
	if (result)
	    bb_error_msg_and_die("%s (%s) failed, exit code %d", script_type, script_path, result);
}
 ...
 * scanning the status file. The resulting list, however, is unsorted. If filter
 * is specified then only package equal to the filter is printed.
 */
static void list_packages(const char *filter)
{
        int i;
	...
	for (i=0; i<STATUS_HASH_PRIME+1; i++) {
			...
			if (filter && strcmp(filter, name_str))
			    continue;
...
void remove_package(const unsigned int package_num, int noisy)
{
	...
	/* run prerm script */
	run_package_script_or_die(package_name, "prerm", "remove");
	...
	run_package_script_or_die(package_name, "postrm", "remove");
...
void purge_package(const unsigned int package_num)
{
	...
	exclude_files = xmalloc(sizeof(char*) * 3);
	exclude_files[0] = bb_xstrdup(list_name);
	exclude_files[1] = xmalloc(strlen(package_name) + 27);
	sprintf(exclude_files[1], "/var/lib/dpkg/info/%s.postrm", package_name);
	exclude_files[2] = NULL;
	remove_file_array(remove_files, exclude_files);
	free(exclude_files);
	...
	/* run postrm script */
	run_package_script_or_die(package_name, "postrm", "remove");
	run_package_script_or_die(package_name, "postrm", "purge");
...
/* This function get the root directory for extracting data.tar.gz */
static char *root_directory_get(void)
{
    char *root = getenv("DPKG_ROOT_DIR");

    if (!root)
    {
	printf("$DPKG_ROOT_DIR is not set, using default (\"%s\")\n", 
	    DEFAULT_ROOT_DIR);	   
...
#endif
#ifdef CONFIG_FEATURE_DEB_TAR_LZMA
	ar_handle->accept = llist_add_to(ar_handle->accept, "control.tar.lzma");
#endif
...
#endif
#ifdef CONFIG_FEATURE_DEB_TAR_LZMA
	ar_handle->accept = llist_add_to(ar_handle->accept, "data.tar.lzma");
#endif
...
static void unpack_package(deb_file_t *deb_file, int skip_same)
{
	...
	if (strcmp(name_hashtable[get_status(status_num, 3)], "installed") == 0) {
	        ...
	        deb_file->ignore = skip_same && test_version(package_hashtable[deb_file->package]->version, 
			package_hashtable[status_package_num]->version, VER_EQUAL);

		if (deb_file->ignore) {
			printf("Version %s of %s already installed, skipping.\n", 
				name_hashtable[package_hashtable[status_package_num]->version], package_name);
			return;
		}
	...
	run_package_script_or_die(package_name, "preinst", "install");

	/* Extract data.tar.gz to the root directory */
	archive_handle = init_archive_deb_ar(deb_file->filename);
	init_archive_deb_data(archive_handle);
	archive_handle->sub_archive->action_data = data_extract_all_prefix;
	archive_handle->sub_archive->buffer = root_directory_get();
	archive_handle->sub_archive->flags |= ARCHIVE_EXTRACT_UNCONDITIONAL;
	unpack_ar_archive(archive_handle);
...
void configure_package(deb_file_t *deb_file)
{
	...
	run_package_script_or_die(package_name, "postinst", "configure");
...
int dpkg_main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "CEF:ilPru")) != -1) {
		switch (opt) {
			case 'C': // equivalent to --configure in official dpkg
				dpkg_opt |= dpkg_opt_configure;
				dpkg_opt |= dpkg_opt_package_name;
				break;
			case 'E': // equivalent to --skip-same-version in official dpkg
				dpkg_opt |= dpkg_opt_skip_same_version;
				break;
...
/*	puts("(Reading database ... xxxxx files and directories installed.)"); */
	...
	while (optind < argc) {
		...
		if (dpkg_opt & dpkg_opt_filename) {
			...
			else if (dpkg_opt & dpkg_opt_list_installed) {
			        list_packages(name_hashtable[package_hashtable[package_num]->name]);
			}
		}
		deb_file[deb_count]->ignore = 0;
		deb_count++;
		optind++;
	}
	...
	for (i = 0; i < deb_count; i++) {
		int skip_same = dpkg_opt & dpkg_opt_skip_same_version;
		...
		if (dpkg_opt & dpkg_opt_remove) {
			...
			unpack_package(deb_file[i], skip_same);
		}

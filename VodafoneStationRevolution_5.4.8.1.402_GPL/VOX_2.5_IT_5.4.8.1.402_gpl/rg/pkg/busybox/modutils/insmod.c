...
static void print_insmod_command(char *m_name, long text, long main_flow,
    long data, long bss)
{
	char buf[1024];
	char *p = buf;
	...
	if (text)
	{
		p += sprintf(p, "insmod: add-symbol-file build/debug/%s.o %#lx",
			m_name, text);
	}
	else
		p += sprintf(p, "insmod: no text section for module %s", m_name);
	if (main_flow)
		p += sprintf(p, " -s .main_flow %#lx", main_flow);
	if (data)
		p += sprintf(p, " -s .data %#lx", data);
	if (bss)
		p += sprintf(p, " -s .bss %#lx", bss);
...
static int obj_relocate(struct obj_file *f, ElfW(Addr) base, char *m_name)
{
	...
	if (!cCONFIG_RG_INSMOD_SILENT)
	{
		...
		print_insmod_command(m_name, text, main_flow, data, bss);
	}

...
static unsigned int str2mark(char *str)
{
	static struct {
		char *name;
		unsigned int so_mark;
	} predefined_marks[] = {
#ifdef MARK_VF_IT_VOICE 
		{ MARK_VF_IT_VOICE , 0x800000 },
#endif
#ifdef MARK_VF_UK_VOICE 
		{ MARK_VF_UK_VOICE , 0x800000 },
#endif
		{ NULL, 0 }
	};
	unsigned int mark;
	...
	for (i = 0; predefined_marks[i].name; i++) {
		if (!strcmp(str, predefined_marks[i].name))
			return predefined_marks[i].so_mark;
	}
...
static inline int tftp(const int cmd, const struct hostent *host,
    ...
    int tftp_bufsize, unsigned int mark)
{
	...
	if (mark && setsockopt(socketfd, SOL_SOCKET, SO_MARK, &mark,
		sizeof(mark)))
	{
		bb_perror_msg("Can't mark socket as %d", mark);
		return EXIT_FAILURE;
	}
...
#define MARK
	...
	while ((opt = getopt(argc, argv, MARK BS GET PUT "l:r:")) != -1) {
		switch (opt) {
#ifdef CONFIG_FEATURE_TFTP_BLOCKSIZE
		case 'b':
	...
	result = tftp(cmd, host, remotefile, fd, port, blocksize, so_mark);

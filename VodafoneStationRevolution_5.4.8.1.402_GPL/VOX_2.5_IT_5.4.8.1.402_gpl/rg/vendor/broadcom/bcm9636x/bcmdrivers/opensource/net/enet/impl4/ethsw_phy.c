...
extern atomic_t phy_access_ref_cnt;
...
void ethsw_phy_access_disable_interrupt(void)
{
    ...
    if (atomic_read(&phy_access_ref_cnt) == 0)
    {
...
void ethsw_phy_access_enable_interrupt(void)
{
    ...
    spin_lock_irqsave(&bcm_ethlock_phy_access, flags);
    if (atomic_dec_and_test(&phy_access_ref_cnt)) {
#if !defined(CONFIG_BCM96818)
...
EXPORT_SYMBOL(ethsw_phy_read_reg);
...
EXPORT_SYMBOL(ethsw_phy_write_reg);

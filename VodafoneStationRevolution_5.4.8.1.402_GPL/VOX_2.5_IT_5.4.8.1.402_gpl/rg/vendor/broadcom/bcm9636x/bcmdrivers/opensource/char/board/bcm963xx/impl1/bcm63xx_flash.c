...
int brcmnand_auto_markbad = 0; // automatically mark blocks bad if write fails
EXPORT_SYMBOL(brcmnand_auto_markbad);
...
static int nandNvRamSet(const char *string, int strLen, int offset)
{
    int retlen;
    ...
    if (IS_ERR(mtd))
    {
	printk("%s: Error getting the nvram mtd device\n", __FUNCTION__);
	ret = -ENODEV;
	mtd = NULL;
	goto Exit;
    }

    if (!(block_buf = kmalloc(mtd->erasesize, GFP_KERNEL)))
    {
	ret = -ENOMEM;
	goto Exit;
    }

    ret = mtd->read(mtd, 0, mtd->erasesize, &retlen, block_buf);
    if ((ret && ret != -EUCLEAN) || retlen != mtd->erasesize)
    {
	if (!ret)
	    ret = -EIO;
	printk("%s: Error reading NVRAM block\n", __FUNCTION__);
	goto Exit;
    }
    memcpy(block_buf + NVRAM_DATA_OFFSET + offset, string, strLen);

    if ((ret = nandEraseBlkNotSpare(mtd, 0)))
    {
	printk("%s: Error erasing NVRAM block. CFEROM & NVRAM are lost!\n",
	    __FUNCTION__);
	goto Exit;
    }

    if ((ret = mtd->write(mtd, 0, mtd->erasesize, &retlen, block_buf)) ||
	retlen != mtd->erasesize)
    {
	if (!ret)
	    ret = -EIO;
	printk("%s: Error writing NVRAM block. CFEROM & NVRAM are lost!\n",
	    __FUNCTION__);
	goto Exit;
    }

    /* Update the saved copy too */
    updateInMemNvramData(string, strLen, 0);
...
int kerSysNvRamSet(const char *string, int strLen, int offset)
{
    ...
    if (bootFromNand == 0)
    {
        ...
        sts = nandNvRamSet(string, strLen, offset);
...
/* based on commit 8018ab057480974e7f26a387bf4ce040e9a5f6f1 from linux git */
static int vfs_fsync(struct file *file, int datasync)
{
    ...
    int partition)
{
    ...
    /* Make sure that rootfs1 (rgloader) seq is 1 and rootfs2 (BSP) seq is 0 */
    if (partition == 1)
    {
	seq = 0;
	seq2 = -1;
    ...
    int should_yield )
{
    ...
    if( mtd0 && mtd1 )
    {
        ...
        cferam_blk = nandUpdateSeqNum((unsigned char *) string + ofs,
            img_size - ofs, mtd0->erasesize,
	    !!strcmp(rootfs_part, "rootfs1") + 1) * mtd0->erasesize;

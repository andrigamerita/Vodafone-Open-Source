    ...
    pNBuff_t pNBuff, int flush )
{
	...
	if (pDevCtx->pVcc->pop)
	    pDevCtx->pVcc->pop(pDevCtx->pVcc, skb);
	else
	    dev_kfree_skb(skb);
    }
    else
...
static int DoOpenConnection( PXTMRT_OPEN_CONNECTION pOc )
{
	...
	for( i = 0; i < MAX_DEV_CTXS; i++ )
		...
		/* XXX: Parameters are not known yet, they will be reconfigured
		 * when we get information regarding encapsulation type from
		 * user mode (at atm backend binding).
		 * Until then, no packets are expected to be received or sent */

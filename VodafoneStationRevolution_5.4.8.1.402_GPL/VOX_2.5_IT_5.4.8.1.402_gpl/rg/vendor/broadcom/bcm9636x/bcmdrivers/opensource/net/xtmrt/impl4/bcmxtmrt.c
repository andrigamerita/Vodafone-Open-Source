...
static int bcmxtmrt_hard_start_xmit( pNBuff_t pNBuff, struct net_device *dev );
...
#else
static void bcmxtmrt_sched_rxtask(void);
#endif
...
static void bcmxtmrt_rx_tasklet( unsigned long data);
...
static int DoCreateDeviceReq( PBCMXTMRT_DEV_CONTEXT pDevCtx, PXTMRT_CREATE_NETWORK_DEVICE pCnd );
...
static int DoOpenConnection ( PXTMRT_OPEN_CONNECTION pOc );
static int DoCloseConnection (PBCMXTMRT_DEV_CONTEXT pDevCtx );
...
static const struct net_device_ops bcmXtmRt_netdevops = {
    ...
    .ndo_start_xmit     = (HardStartXmitFuncP)bcmxtmrt_hard_start_xmit,
...
static DECLARE_TASKLET(rx_tasklet, bcmxtmrt_rx_tasklet, 0);

static inline void release_nbuff( PBCMXTMRT_DEV_CONTEXT pDevCtx, 
    pNBuff_t pNBuff, int flush )
{
    if (pDevCtx->pVcc)
    {
	struct sk_buff *skb = PNBUFF_2_SKBUFF(pNBuff);
	/* This device context is managed by linux ATM, therefore
	 * nBuffs are skb's, and we need to call pop() */
	if (flush)
...
int __init bcmxtmrt_init( void )
{
    ...
    g_GlobalInfo.pRxTrigger = &bcmxtmrt_sched_rxtask;
#endif
#if (defined(CONFIG_BCM_BPM) || defined(CONFIG_BCM_BPM_MODULE))
        ...
        if( get_user(b, (atm_backend_t __user *) argp) == 0 )
        {
            ...
            case ATM_BACKEND_PPP_BCM_DISCONN:
                ...
                if((pDevCtx = FindDevCtx(pAtmVcc->vpi, pAtmVcc->vci)) != NULL)
                {
                    ...
                    nRet = bcmxtmrt_xmit( SKBUFF_2_PNBUFF(skb), pDevCtx );
                }
                else
                    nRet = -EFAULT;
                break;
...
static int bcmxtmrt_pppoatm_send(struct ppp_channel *pChan, struct sk_buff *skb)
{
    ...
    bcmxtmrt_hard_start_xmit( SKBUFF_2_PNBUFF(skb), (struct net_device *) pChan->private );
    return(1);
...
static int bcmxtmrt_hard_start_xmit( pNBuff_t pNBuff, struct net_device *dev )
{
    ...
    PBCMXTMRT_DEV_CONTEXT pDevCtx = dev->priv;
#endif
    return bcmxtmrt_xmit( pNBuff, pDevCtx );
}
 ...
 * Returns      : 0 if successful or error status
 ***************************************************************************/
int bcmxtmrt_xmit( pNBuff_t pNBuff, PBCMXTMRT_DEV_CONTEXT pDevCtx )
{
    PBCMXTMRT_GLOBAL_INFO pGi = &g_GlobalInfo;
    DmaDesc  dmaDesc;
    uint32 dqm;
    ...
    if( pDevCtx->ulLinkState == LINK_UP )
    {
        ...
        for (i=0; i<pDevCtx->ulTxQInfosSize; i++)
        {
		    ...
		    release_nbuff(pDevCtx, nbuff_reclaim_p, 0);
                    spin_lock_bh(&pGi->xtmlock_tx);
                }
            }
        }
...
tx_continue:
		...
		if (pDevCtx->pDev)
            ...
            else
            {
                 ...
                 * queue.
                 */
		release_nbuff(pDevCtx, pNBuff, 1);
                pDevCtx->DevStats.tx_dropped++;
            }
        }
        else
...
#else
/* Allows triggering RX tasklet by pktdma DQM code when using pVcc */
static void bcmxtmrt_sched_rxtask(void)
{
    tasklet_schedule(&rx_tasklet);
}
...
static int bcmxtmrt_poll(struct net_device * dev, int * budget)
{
    ...
    UINT32 ulMask;
    UINT32 i;
    UINT32 work_to_do = dev ? min(dev->quota, *budget) : *budget;
    UINT32 work_done;
    UINT32 ret_done;
    UINT32 more_to_do = 0;
    UINT32 flags;
                                 ...
                                 FkBuff_t *pFkb, UINT16 bufStatus, int delLen, int trailerDelLen)
{
...
#else
   {
	...
	if (pDevCtx->pDev)
	{
		...
		/*PPPoA*/
#ifdef CONFIG_BCM_ATM_BACKEND_PPP_BCM
	...
	{
	    ATM_SKB(skb)->vcc = pDevCtx->pVcc;
	    if (!atm_charge(pDevCtx->pVcc, skb->truesize))
	    {
		dev_kfree_skb(skb);
		pDevCtx->DevStats.rx_dropped++;
	    }
	    else
		pDevCtx->pVcc->push(pDevCtx->pVcc, skb);
	}
...
static void bcmxtmrt_timer( PBCMXTMRT_GLOBAL_INFO pGi )
{
    ...
    for( i = 0; i < MAX_DEV_CTXS; i++ ) {
        if( (pDevCtx = pGi->pDevCtxs[i]) ) {
            if( pDevCtx->ulTxQInfosSize )
            {
                bcmxtmrt_xmit( PNBUFF_NULL, pGi->pDevCtxs[i] );
            }
...
int bcmxtmrt_request( XTMRT_HANDLE hDev, UINT32 ulCommand, void *pParm )
{
   ...
   switch( ulCommand )
   {
      ...
      case XTMRT_CMD_CREATE_DEVICE:
         nRet = DoCreateDeviceReq( pDevCtx, (PXTMRT_CREATE_NETWORK_DEVICE) pParm );
         break;
      ...
      case XTMRT_CMD_OPEN_CONNECTION:
	 nRet = DoOpenConnection( (PXTMRT_OPEN_CONNECTION) pParm );
	 break;

      case XTMRT_CMD_CLOSE_CONNECTION:
	 nRet = DoCloseConnection( pDevCtx );
	 break;
...
static int DoOpenConnection( PXTMRT_OPEN_CONNECTION pOc )
{
    ...
    if( pGi->ulDrvState != XTMRT_UNINITIALIZED )
    {
	if (!(pDevCtx = kmalloc( sizeof(BCMXTMRT_DEV_CONTEXT), GFP_KERNEL)))
	    return -ENOMEM;

	memset(pDevCtx, 0x00, sizeof(BCMXTMRT_DEV_CONTEXT));
        memcpy(&pDevCtx->Addr, &pOc->ConnAddr, sizeof(XTM_ADDR));
        if(( pOc->ConnAddr.ulTrafficType & TRAFFIC_TYPE_ATM_MASK ) == TRAFFIC_TYPE_ATM )
            pDevCtx->ulHdrType = pOc->ulHeaderType;
        else
...
static int DoCloseConnection( PBCMXTMRT_DEV_CONTEXT pDevCtx )
{
    ...
    BCM_XTM_DEBUG("DoCloseConnection\n");
...
static int DoCreateDeviceReq( PBCMXTMRT_DEV_CONTEXT pDevCtx, PXTMRT_CREATE_NETWORK_DEVICE pCnd )
{
    ...
    if( (dev = alloc_netdev( 0, pCnd->szNetworkDeviceName, ether_setup )) != NULL )
    {
...
#error fix me
	...
	pDevCtx->pDev = dev;
        ...
        dev->hard_start_xmit    = (HardStartXmitFuncP) bcmxtmrt_hard_start_xmit;
        ...
        default:
            dev->flags = IFF_BROADCAST | IFF_MULTICAST; /* bridge, MER, PPPoE, PTM */
            break;
        }
...
static int DoSendCellReq( PBCMXTMRT_DEV_CONTEXT pDevCtx, PXTMRT_CELL pC )
{
    ...
    if( pDevCtx->ulLinkState == LINK_UP )
    {
		...
		atomic_add(skb->truesize,
		    &sk_atm(pDevCtx->pVcc)->sk_wmem_alloc);
	    }

	    ATM_SKB(skb)->vcc = pDevCtx->pVcc;

            bcmxtmrt_xmit( SKBUFF_2_PNBUFF(skb), pDevCtx );
        }
        else
            nRet = -ENOMEM;
    }
...
EXPORT_SYMBOL(bcmxtmrt_xmit);
EXPORT_SYMBOL(FindDevCtx);
...
EXPORT_SYMBOL(bcmxtmrt_get_xmit_channel);

/* Gets the RX channel for the device, based on the assumption that there is
 * only one RX channel and that pDevCtx->ucTxVcid == ucRxVcids[0] */
int bcmxtmrt_get_rx_channel(void *dev_p, int channel)
{
    struct net_device *dev = (struct net_device *)dev_p;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30)
    PBCMXTMRT_DEV_CONTEXT pDevCtx = netdev_priv(dev);
    ...
    if (pDevCtx->ucTxVcid != INVALID_VCID)
        channel = pDevCtx->ucTxVcid;

    return channel;
}
EXPORT_SYMBOL(bcmxtmrt_get_rx_channel);

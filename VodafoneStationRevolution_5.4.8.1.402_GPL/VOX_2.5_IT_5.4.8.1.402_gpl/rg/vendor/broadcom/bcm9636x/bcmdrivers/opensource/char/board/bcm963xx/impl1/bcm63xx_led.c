...
static BP_LED_INFO bpLedInfo[] =
{
    ...
    {kLedPwr, BpGetBootloaderPowerOnLedGpio, NULL},
    {kLedEnd, NULL, NULL}
};
...
void kerLedGpioStateInit(void);
EXPORT_SYMBOL(kerLedGpioStateInit);
...
void __init boardLedInit(void)
{
	...
	/* No reason to switch Power Led by default */
	if (pInfo->ledName == kLedPwr)
	    continue;
...
void kerLedGpioStateInit(void)
{
    ...
    //config led
    GPIO->GPIOMode &= ~GPIO_NUM_TO_MASK(BP_GPIO_0_AH);
    GPIO->GPIOMode &= ~GPIO_NUM_TO_MASK(BP_GPIO_1_AH);

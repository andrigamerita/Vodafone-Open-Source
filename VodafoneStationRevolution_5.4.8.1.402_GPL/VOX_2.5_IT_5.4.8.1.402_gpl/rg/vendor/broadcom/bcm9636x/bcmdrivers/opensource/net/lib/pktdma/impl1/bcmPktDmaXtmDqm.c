...
void bcm63xx_xtm_dqmhandler(uint32 fapIdx, unsigned long channel)
{
    ...
    for (i = 0; i < MAX_DEV_CTXS; i++)
    {
        if ((pDevCtx = g_pXtmGlobalInfo->pDevCtxs[i]) != NULL)
        {
            ...
            {
                if (likely(g_pXtmGlobalInfo->pRxTrigger))
                    g_pXtmGlobalInfo->pRxTrigger();
                scheduled = 1;
            }
        }
    }
    ...
    if (scheduled)
    {
        ...
        spin_lock(&g_pXtmGlobalInfo->xtmlock_rx_regs);
        g_pXtmGlobalInfo->ulIntEnableMask |= 1 << channel;
        spin_unlock(&g_pXtmGlobalInfo->xtmlock_rx_regs);
    }
}

...
brcmnanddrv_setup_mtd_partitions(struct brcmnand_info* nandinfo)
{
	...
	int i = 0;
        ...
        bcm63XX_nand_parts[i].name = "rootfs";
        bcm63XX_nand_parts[i].offset = nvram.ulNandPartOfsKb[rootfs] * 1024;
        bcm63XX_nand_parts[i].size = nvram.ulNandPartSizeKb[rootfs] * 1024;
        bcm63XX_nand_parts[i].ecclayout = mtd->ecclayout;
	i++;
	if (nvram.ulNandPartSizeKb[rootfs_update])
	{
	    bcm63XX_nand_parts[i].name = "rootfs_update";
	    bcm63XX_nand_parts[i].offset = 
		nvram.ulNandPartOfsKb[rootfs_update] * 1024;
	    bcm63XX_nand_parts[i].size = 
		nvram.ulNandPartSizeKb[rootfs_update] * 1024;
	    bcm63XX_nand_parts[i].ecclayout = mtd->ecclayout;
	    i++;
	}
#endif
        ...
        bcm63XX_nand_parts[i].name = "data";
        bcm63XX_nand_parts[i].offset = nvram.ulNandPartOfsKb[NP_DATA] * 1024;
        bcm63XX_nand_parts[i].size = nvram.ulNandPartSizeKb[NP_DATA] * 1024;
        bcm63XX_nand_parts[i].ecclayout = mtd->ecclayout;
	i++;

        /* Boot and NVRAM data */
        bcm63XX_nand_parts[i].name = "nvram";
        bcm63XX_nand_parts[i].offset = nvram.ulNandPartOfsKb[NP_BOOT] * 1024;
        bcm63XX_nand_parts[i].size = nvram.ulNandPartSizeKb[NP_BOOT] * 1024;
        bcm63XX_nand_parts[i].ecclayout = mtd->ecclayout;
	i++;
	...
	{
	    PRINTK("Part[%d] name=%s, size=%x, ofs=%x\n", i, 
		bcm63XX_nand_parts[i].name, 
		bcm63XX_nand_parts[i].size, bcm63XX_nand_parts[i].offset);
	}
    }
}
...
static int __devinit brcmnanddrv_probe(struct platform_device *pdev)
{
    ...
    else
    {
		...
		info->mtd.name = pdev->dev.bus_id;
			...
			const char *parser_names[] = { "cmdlinepart", NULL };
			int n;

			/* Select dynamic from cmdline partition definitions */
			n = parse_mtd_partitions(&info->mtd, parser_names, &mtd_parts, 0);

			if (n > 0)
			    add_mtd_partitions(&info->mtd, mtd_parts, n);
		    }
#endif

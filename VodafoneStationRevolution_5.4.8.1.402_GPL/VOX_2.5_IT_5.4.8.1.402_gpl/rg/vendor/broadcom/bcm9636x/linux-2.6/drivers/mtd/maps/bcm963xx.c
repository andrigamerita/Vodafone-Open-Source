...
static struct mtd_erase_region_info merPhysMap = {
    .offset = 0,
    .erasesize = 0,
    .numblocks = 0
};

static struct mtd_info mtdPhysMap = {
    .name		= CONFIG_RG_PERM_STORAGE_DEFAULT_PARTITION,
    .index		= -1,
    .type		= MTD_NORFLASH,
    .flags		= MTD_CAP_NORFLASH,
    .numeraseregions= 1,
    .eraseregions	= (struct mtd_erase_region_info*) &merPhysMap,
    .writesize       = 1,
    .read		= bcm63xx_read,
    .erase		= bcm63xx_erase,
    .write		= bcm63xx_write, 
...
static int __init init_brcm_physmap(void)
{
    mtdPhysMap.priv = flash_get_memptr(0);
    mtdPhysMap.size = flash_get_total_size();
    mtdPhysMap.erasesize = flash_get_sector_size(0);
    mtdPhysMap.numeraseregions = 1;
    mtdPhysMap.eraseregions->offset = 0;
    mtdPhysMap.eraseregions->erasesize = flash_get_sector_size(0);
    mtdPhysMap.eraseregions->numblocks = flash_get_numsectors();

    if (add_mtd_device(&mtdPhysMap))
    {
	printk("Failed to register device mtd[%s]\n", mtdPhysMap.name);
	return -EIO;
    }
    ...
    printk("Registered device mtd[%s] dev[%d] Flash[0x%08x,%llu]\n",
	mtdPhysMap.name, mtdPhysMap.index,
	(int)mtdPhysMap.priv, (unsigned long long)mtdPhysMap.size);
...
static void __exit cleanup_brcm_physmap(void)
{
    if (mtdPhysMap.index < 0)
	return;

    mtdPhysMap.index = -1;
    del_mtd_device(&mtdPhysMap);
}
#endif

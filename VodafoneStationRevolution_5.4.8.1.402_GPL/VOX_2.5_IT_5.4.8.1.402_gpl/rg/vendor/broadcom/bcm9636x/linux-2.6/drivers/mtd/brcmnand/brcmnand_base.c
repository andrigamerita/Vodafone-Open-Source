 ...
 * we want to handle mtd read requests as soon as possible, but they
 * should not lock up the box. */
#define MAX_READ_TIME msecs_to_jiffies(10)
	    	unsigned long start_reading = jiffies, max = MAX_READ_TIME;
		...
		while(1) {
			...
			if (time_after(jiffies, start_reading + max) && !wr_preempt_en && !in_interrupt())
			{
			    cond_resched();
			    start_reading = jiffies;
			}
		}
	}

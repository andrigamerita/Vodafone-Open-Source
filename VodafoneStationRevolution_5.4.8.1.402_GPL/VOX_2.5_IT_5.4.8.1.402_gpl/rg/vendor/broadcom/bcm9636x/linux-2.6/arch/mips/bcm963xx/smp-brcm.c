...
static unsigned int c0expire[NR_CPUS];
...
#endif
extern unsigned long cycles_per_jiffy __read_mostly;
...
#endif
{
	...
	c0expire[cpu] = read_c0_count() + cycles_per_jiffy;
	write_c0_compare(c0expire[cpu]);
}
...
/* brcm_timer_interrupt from BSP 4.04L.02 */

// Duplicate standard mips timer ack code here for smp
void brcm_c0_timer_ack(void)
{
	int cpu = smp_processor_id();
	unsigned int count;

	/* Ack this timer interrupt and set the next one.  */
	c0expire[cpu] += cycles_per_jiffy;
	write_c0_compare(c0expire[cpu]);

	/* Check to see if we have missed any timer interrupts.  */
	while (((count = read_c0_count()) - c0expire[cpu]) < 0x7fffffff) {
		c0expire[cpu] = count + cycles_per_jiffy;
		write_c0_compare(c0expire[cpu]);
	}
}
...
irqreturn_t brcm_timer_interrupt(int irq, void *dev_id)
{
	...
	if (smp_processor_id() == 0) {
		...
		/* For CPU 0 this will be called inside of timer_interrupt()
		 * to allow it to work without CONFIG_SMP. */
#if defined(CONFIG_BCM_HOSTMIPS_PWRSAVE) || defined(CONFIG_BCM_DDR_SELF_REFRESH_PWRSAVE)

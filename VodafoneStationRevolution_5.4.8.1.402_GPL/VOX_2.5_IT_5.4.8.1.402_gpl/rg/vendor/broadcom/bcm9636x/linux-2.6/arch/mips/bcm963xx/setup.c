...
void __init plat_timer_setup(struct irqaction *irq)
{
    ...
    irq->handler = brcm_timer_interrupt;
#endif

    /* XXX for now. NO PM Stuff */
...
#else
    /* cp0 counter/compare (IE_IRQ5) gets enabled at end of plat_timer_setup */
#endif
}
...
typedef struct {
    char *board_type;
    char *plat_devs[2];
    int lcd_type; /* only valid if non-zero */
} plat_dev_t;

static const plat_dev_t __initdata const supported_pd[] = {
     /* sercomm ywzoob original board name uses bcm's board name */
    {"96362ADVNgr", {"fb_hx8347i", "tb_sx8646"}, 5},
    {"96362ADVNgr", {"fpga_fb", "tb_sx8646"}},
     /* sercomm ywzoob suggested name */
    {"96362YWZ00", {"fpga_fb", "tb_sx8646"}},
     /* huawei hg558 */
    {"6362hhb3",  {"fb_hx8347b", "tb_so380000"}},
    {NULL}
};

static int __init bcm63xx_universal_hw_init(void)
{
    ...
    for (i = 0; supported_pd[i].board_type; i++)
    {
	if (strcmp(board_id, supported_pd[i].board_type))
	    continue;

	if (supported_pd[i].lcd_type &&
	    supported_pd[i].lcd_type != kerSysGetLcdType())
	{
	    continue;
	}
	for (j = 0; j < ARRAY_SIZE(supported_pd[i].plat_devs); j++)
	{
	    printk("Add platform device \"%s\"\n", supported_pd[i].plat_devs[j]);
	    pd = platform_device_alloc(supported_pd[i].plat_devs[j], 0);
	    if (!pd)
		return -ENOMEM;
...
arch_initcall(bcm63xx_universal_hw_init);
...
static int __init bcm63268_hw_init(void)
{
    ...
    USBH->Setup |= USBH_IOC;
    USBH->Setup &= ~USBH_IPP;
    /* Keep power down for 800ms to ensure a connected dongle has settled. See
     * B185473 */
    mdelay(800);
    USBH->PllControl1 &= ~(PLLC_PLL_IDDQ_PWRDN | PLLC_PLL_PWRDN_DELAY);
#else
...
unsigned long getMemorySize(void)
{
    ...
    return(64 * 1024 * 1024); 
#elif defined(CONFIG_BRCM_MEMORY_RESTRICTION_96M)
    return(96 * 1024 * 1024);

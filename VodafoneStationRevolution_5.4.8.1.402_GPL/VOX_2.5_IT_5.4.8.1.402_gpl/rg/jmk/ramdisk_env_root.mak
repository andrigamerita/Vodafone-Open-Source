...
ifdef CONFIG_RG_JSMIN
  RAMDISK_STRIP_JS={ $(JSMIN) $1 && echo '- minifing $1' >> \
    $(RAMDISK_REPORT_FILE); }
else
  RAMDISK_STRIP_JS=true
endif
ifdef CONFIG_RG_COMPRESSED_JS
  COMPRESS_CMD=$(GZIP) --to-stdout --no-name --quiet --best
  RAMDISK_COMPRESS_JS={ $(COMPRESS_CMD) $1 > ${1}.gz && \
    echo '- compressing $1' >> $(RAMDISK_REPORT_FILE); }
else
  RAMDISK_COMPRESS_JS=true
endif
RAMDISK_PROCESS_JS=$(RAMDISK_STRIP_JS) && $(RAMDISK_COMPRESS_JS)
...
RAMDISK_CP_RO_FUNC= \
  $(call _RAMDISK_CP_RO_FUNC,$1,$2,$3,$(JMKE_RAMDISK_RO_DIR),$(JMKE_MAINFS_RUNTIME_MOUNT_POINT))

RAMDISK_CP_MOD_FUNC= \
  $(call _RAMDISK_CP_RO_FUNC,$1,$2,$3,$(MODULES_DIR),$(MODULES_RUNTIME_MOUNT_POINT))

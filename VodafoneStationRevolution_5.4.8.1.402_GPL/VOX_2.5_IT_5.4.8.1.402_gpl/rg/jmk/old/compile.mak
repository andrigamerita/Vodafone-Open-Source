...
_GET_COMPILER=$(call DBGFR,\
  $(if $(call IS_LOCAL_OBJ,$2),\
    $(CC_FOR_BUILD),\
    $(if $(call GET_MODULE,$2)$(filter $2,$(JMK_TARGET_CC_OBJS)),\
      $(TARGET_CC),\
...
$(JMK_O_TARGET):
	$(if $(JMK_JPKG_TARGET_$@),$(LD) $(ENDIAN_LDFLAGS) -r -o $@ $(foreach o,$(filter-out $(JMKE_PWD_SRC)/Makefile,$^), $(call OS_PATH,$(o))) $(JMK_LDFLAGS_$@) && $(JMKE_LN) $(JMKE_PWD_BUILD)/$@ $(DEBUG_PATH)/$@)
...
$(JMK_MOD_TARGET):
	...
	$(if $(JMK_JPKG_TARGET_$@),\
	  $(if $(filter $@,$(JMK_MOD_2_STAT) $(JMK_MOD_O_TARGET)),, \
	    $(JMKE_LN) $(JMK_ROOT)/pkg/util/kext_mod.c $(*F).mod.c && \
	    $(TARGET_CC) $(JMK_RG_LINUX_CFLAGS) $(JMK_MOD_CFLAGS) $(MOD_LINUX_CFLAGS) \
	      -DMODULE -c -o $(*F).mod.o $(*F).mod.c))
	$(if $(JMK_JPKG_TARGET_$@), \
	  $(LD) $(ENDIAN_LDFLAGS) -r -o $@ -Map $@.link.map \
	    $(filter-out $(*F).mod.c, \
	      $(filter-out %.h, $(filter-out $(JMKE_PWD_SRC)/Makefile,$^))) \
	    $(if $(filter $@,$(JMK_MOD_2_STAT) $(JMK_MOD_O_TARGET)),, \
	      $(*F).mod.o $(MOD_LDFLAGS)) \
	    $(JMK_LDFLAGS_$@) && \

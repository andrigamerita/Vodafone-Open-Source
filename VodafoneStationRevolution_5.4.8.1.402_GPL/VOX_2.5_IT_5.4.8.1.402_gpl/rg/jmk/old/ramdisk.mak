...
$(RAMDISK_MODULES_FILES_):
	@$(call $(if $(CONFIG_RG_MODULES_INITRAMFS),RAMDISK_CP_MOD_FUNC,RAMDISK_CP_RO_FUNC),$(notdir $(@:__%_rd=%)),$(@:__%_rd=%),$(if $(filter $(notdir $(@:__%_rd=%)),$(JMK_MODULES_DONT_STRIP)),DONT_STRIP,STRIP_DEBUG_SYM))

#change the module filename from .ko to .o in the ramdisk
# the module file path is relative to the source dir

# $1 - list of modules
JMKF_ADD_RAMDISK_KERNEL_MODULES= \
  $(foreach f,$(filter-out $(JMK_MOD_2_STAT) $(JMK_MOD_O_TARGET),$1),$(call $(if $(CONFIG_RG_MODULES_INITRAMFS),RAMDISK_CP_MOD_FUNC,RAMDISK_CP_RO_FUNC),$f,/lib/modules/$(basename $(notdir $f)).o,STRIP_DEBUG_SYM) && ) \
  true

...
export MAINFS_DIR=$(DISK_IMAGE_DIR)/mainfs_dir
export MODULES_DIR=$(DISK_IMAGE_DIR)/modules_dir
export JMKE_RAMDISK_RO_DIR=$(MAINFS_DIR)
...
export JMKE_MAINFS_RUNTIME_MOUNT_POINT=/mnt/mainfs
export MODULES_RUNTIME_MOUNT_POINT=/tmp/modules_dir
...
ifdef BOOTLDR_RAMDISK
  ...
  MAINFS_DIR=$(DISK_IMAGE_DIR)/mainfs_dir
  JMKE_RAMDISK_RO_DIR=$(MAINFS_DIR)
...
RAMDISK_CP_RO_FUNC=$(call RAMDISK_MKDIR_FUNC,$(JMKE_RAMDISK_RO_DIR)/$(dir $2)) && \
	      $(call RAMDISK_LN_FUNC,$(JMKE_MAINFS_RUNTIME_MOUNT_POINT)/$2,$(JMKE_RAMDISK_RW_DIR)/$2) && \
	      $(call RAMDISK_CP_FUNC,$1,$(JMKE_RAMDISK_RO_DIR)/$2) && \
	      echo '- $(patsubst $(JMK_ROOT)/%,%,$(CUR_DIR))/$1 -> $(JMKE_MAINFS_RUNTIME_MOUNT_POINT)/$2' >> $(RAMDISK_REPORT_FILE) && \
	      ...
	      $(call DISK_IMAGE_FUNC,$1,$(DISK_IMAGE_DIR)/mainfs_dir$2)
	      
RAMDISK_CP_MOD_FUNC=$(call RAMDISK_MKDIR_FUNC,$(MODULES_DIR)/$(dir $2)) && \
	      $(call RAMDISK_LN_FUNC,$(MODULES_RUNTIME_MOUNT_POINT)/$2,$(JMKE_RAMDISK_RW_DIR)/$2) && \
	      $(call RAMDISK_CP_FUNC,$1,$(MODULES_DIR)/$2) && \
	      echo '- $(patsubst $(JMK_ROOT)/%,%,$(CUR_DIR))/$1 -> $(MODULES_RUNTIME_MOUNT_POINT)/$2' >> $(RAMDISK_REPORT_FILE) && \
	      $(if $(call EQ,$3,DONT_STRIP),true,$(call $(if $(call EQ,$3,STRIP),RAMDISK_STRIP_FUNC,RAMDISK_STRIP_DEBUG_SYM_FUNC),$(MODULES_DIR)/$2)) && \
	      $(call CRAMFS_FILES_FUNC,$1) && \
	      $(call DISK_IMAGE_FUNC,$1,$(DISK_IMAGE_DIR)/modules_dir$2)

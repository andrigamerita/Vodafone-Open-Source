...
# JMK_MOD_CFLAGS is used both in _mod_[3X|26].o and module.o
ifdef WARN2ERR_MARK
  JMK_CFLAGS+=-Werror
...
ifdef CONFIG_RG_OS_LINUX
  MODFLAGS+=-DMODULE
  MOD_LINUX_CFLAGS+=-D"KBUILD_STR(s)=\#s" -D"KBUILD_BASENAME=KBUILD_STR($(@:%.o=%))" -D"KBUILD_MODNAME=KBUILD_STR($(*F))"
endif
...
ifdef CONFIG_RG_OS_LINUX
  GET_MODFLAGS=$(call DBGFR,\
    $(JMK_RG_LINUX_CFLAGS) $(JMK_MOD_CFLAGS) $(JMK_MOD_CFLAGS_$(call FIX_OBJ_NAME,$1)) \
    $(if $(filter $(call GET_MODULE,$1),$(JMK_MOD_2_STAT)),\
      ,\
      $(MODFLAGS) \
    ) \
    $(if $(filter %_mod_3X.o,$1),$(MOD_LINUX_CFLAGS))\
    $(if $(filter %_mod_26.o,$1),$(MOD_LINUX_CFLAGS))\
    $(if $(call IS_XOBJ,$1),-DEXPORT_SYMTAB)\
  ,GET_MODFLAGS,$1)
endif

JMK_MOD_CFLAGS+=-I$(JMK_ROOT)/pkg/include -I$(RG_INCLUDE_RG_LINUX_DIR)
...
_GET_OBJ_CFLAGS=$(call DBGFR,\
  ...
  $(if $(call IS_LOCAL_OBJ,$2),\
    ...
    $(if $(filter $2,$(JMK_TARGET_CC_OBJS)),\
      $(JMK_CFLAGS) -I$(JMKE_BUILDDIR)/pkg/include,\
...
# $2 - obj name
# $3 - src
# linux kernel includes are already fixed in config_host.c, don't fix them here
GET_OBJ_CFLAGS=$(call DBGFR,\
  $(call FIX_VPATH_CFLAGS_EXCEPT_FOR,\
    $(filter-out $(JMK_CFLAGS_REMOVE) $(JMK_CFLAGS_REMOVE_$2),\
      $(if $(filter %_mod_3X.o,$2),\
	$(filter-out $(RG_LINUX_26_CFLAGS),$(call _GET_OBJ_CFLAGS,$1,$2,$3)),\
	$(if $(filter %_mod_26.o,$2),\
	  $(filter-out $(RG_LINUX_3X_CFLAGS),$(call _GET_OBJ_CFLAGS,$1,$2,$3)),\
	  $(if $(filter %.cpp,$3),\
	    $(filter-out -Wstrict-prototypes,$(call _GET_OBJ_CFLAGS,$1,$2,$3)),\
	    $(call _GET_OBJ_CFLAGS,$1,$2,$3))))),\
    $(RG_LINUX_3X_CFLAGS) $(RG_LINUX_26_CFLAGS))\

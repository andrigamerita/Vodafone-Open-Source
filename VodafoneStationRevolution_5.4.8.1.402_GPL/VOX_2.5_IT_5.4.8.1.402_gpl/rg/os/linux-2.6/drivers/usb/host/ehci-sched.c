...
static int enable_periodic (struct ehci_hcd *ehci)
{
	...
	if (status != 0) {
		/* Based on 221f8dfca89276d8aec54c6d07fbe20c281668f0 from
		 * upstream kernel: the schedules do get turned off reasonably
		 * quickly, but some controllers occasionally leave the
		 * schedules' status bits turned on and consequently ehci-hcd
		 * can't tell that the schedules are off.
		 */
		if (status == -ETIMEDOUT) {
			ehci_err(ehci, "Waited too long for the periodic "

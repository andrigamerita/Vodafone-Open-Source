...
#define UHCI_IS_STOPPED		9999		/* Larger than a frame # */
	...
	int	(*remote_wakeup_is_broken) (struct uhci_hcd *uhci);
};
...
static inline u32 uhci_readl(const struct uhci_hcd *uhci, int reg)
{
	...
	if (uhci_big_endian_mmio(uhci))
	    return __swab32(inl(uhci->io_addr + reg));
#endif
...
{
#ifdef CONFIG_USB_UHCI_BIG_ENDIAN_MMIO
	uhci_big_endian_mmio(uhci) ? outl(__swab32(val), uhci->io_addr + reg) :
#endif
...
#ifdef CONFIG_USB_UHCI_BIG_ENDIAN_MMIO
	if (uhci_big_endian_mmio(uhci))
	    return __swab16(inw(uhci->io_addr + reg));
#endif
...
{
#ifdef CONFIG_USB_UHCI_BIG_ENDIAN_MMIO
	uhci_big_endian_mmio(uhci) ? outw(__swab16(val), uhci->io_addr + reg) :
#endif

...
static int serial_read_proc (char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	for (i = 0; i < SERIAL_TTY_MINORS && length < PAGE_SIZE; ++i) {
		...
		length += sprintf (page+length, " path:%s", tmp);
                ...
                length += sprintf (page+length, " id:%d-%d",
                    serial->dev->bus->busnum, serial->dev->devnum);

		/* Update line number just for connected devices */
		if (bno == serial->dev->bus->busnum &&
                    dno == serial->dev->devnum)
		{
			line++;
		}
		else if (serial->dev->devnum >= 0)
                {
			bno = serial->dev->bus->busnum;
			dno = serial->dev->devnum;
			line = 1;
                }

                length += sprintf (page+length, " line:%d", line);

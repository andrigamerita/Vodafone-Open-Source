...
int slram_register_device(char *name, unsigned long start, unsigned long length)
{
	...
	*curmtd = kmalloc(sizeof(slram_mtd_list_t), GFP_KERNEL);
	...
	T("slram: mdtdinfo->priv->start{0x%p} = ",
	    ((slram_priv_t *)(*curmtd)->mtdinfo->priv)->start);
	T("0x%lx\n",
	    *((long *)((slram_priv_t *)(*curmtd)->mtdinfo->priv)->start));
...
static int parse_cmdline(char *devname, char *szstart, char *szlength)
{
	...
	if ((devstart = slram_register_device(devname, devstart, devlength))){
		unregister_devices();
		return((int)devstart);
	}
	return(0);
...
int slram_unregister_device(char *name)
{
	slram_mtd_list_t **cur;
	slram_mtd_list_t *tmp;

	for (cur = &slram_mtdlist;
	    *cur && strcmp((*cur)->mtdinfo->name, name);
	    cur = &((*cur)->next));
	...
	*cur = (*cur)->next;
	...
	del_mtd_device(tmp->mtdinfo);
	iounmap(((slram_priv_t *)tmp->mtdinfo->priv)->start);
	kfree(tmp->mtdinfo->priv);
	kfree(tmp->mtdinfo);
	kfree(tmp);
	return 0;
}

EXPORT_SYMBOL(slram_register_device);
EXPORT_SYMBOL(slram_unregister_device);

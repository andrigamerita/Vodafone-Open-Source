...
static void ndisc_raflags_notify(struct inet6_dev *inet6_dev, int managed,
    int otherconf)
{
	struct sk_buff *skb;
	struct nlmsghdr *nlh;
	struct raflagsmsg *raflmsg;
	int err;
	size_t msg_size = NLMSG_ALIGN(sizeof(struct raflagsmsg));
	...
	nlh = nlmsg_put(skb, 0, 0, RTM_RAFLAGS, msg_size, 0);
	if (nlh == NULL)
		goto nla_put_failure;

	raflmsg = nlmsg_data(nlh);
	raflmsg->rafl_family = AF_INET6;
	raflmsg->rafl_flags = (managed ? IF_RA_MANAGED : 0) | (otherconf ?
	    IF_RA_OTHERCONF : 0);
	raflmsg->rafl_index = inet6_dev->dev->ifindex;

	nlmsg_end(skb, nlh);

	err = rtnl_notify(skb, 0, RTNLGRP_IPV6_RAFLAGS, NULL, GFP_ATOMIC);
	if (err < 0)
		goto errout;
...
nla_put_failure:
	...
	err = -EMSGSIZE;
errout:
	rtnl_set_sk_err(RTNLGRP_IPV6_RAFLAGS, err);
}
...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	in6_dev->if_flags = (in6_dev->if_flags & ~(IF_RA_MANAGED |
				...
				(ra_msg->icmph.icmp6_addrconf_other ?
					IF_RA_OTHERCONF : 0);
	ndisc_raflags_notify(in6_dev, ra_msg->icmph.icmp6_addrconf_managed,
	    ra_msg->icmph.icmp6_addrconf_other);

...
static void skb_dump(struct sk_buff *skb)
{
	...
	printk(KERN_ERR "skb=%p skb_shinfo(skb)=%p nr_frags=%d dataref=%d "
		"nohdr=%d cloned=%d protocol=%x dev=%p users=%d frag_list=%p "
		"iif=%d dst=%p len=%d data_len=%d mac_len=%d next=%p prev=%p "
		"sk=%p h.raw=%p nh.raw=%p mac.raw=%p priority=%d local_df=%d "
		"ip_summed=%d nfctinfo=%d pkt_type=%d fclone=%d destructor=%p "
		"tc_index=%d phy_port_id=%d bridge=%p mark=%x recycle_hook=%p "
		"recycle_context=%x recycle_flags=%x fp_flow_info=%p "
		"truesize=%u head=%p data=%p tail=%p end=%p\n",
		skb, skb_shinfo(skb), skb_shinfo(skb)->nr_frags,
		atomic_read(&skb_shinfo(skb)->dataref), skb->nohdr, skb->cloned,
		ntohs(skb->protocol), skb->dev, atomic_read(&skb->users),
		skb_shinfo(skb)->frag_list, skb->iif, skb->dst, skb->len,
		skb->data_len, skb->mac_len, skb->next, skb->prev, skb->sk,
		skb->h.raw, skb->nh.raw, skb->mac.raw, skb->priority,
		skb->local_df, skb->ip_summed, skb->nfctinfo, skb->pkt_type,
		skb->fclone, skb->destructor, skb->tc_index, skb->phy_port_id,
		skb->bridge, skb->mark, skb->recycle_hook, skb->recycle_context,
		skb->recycle_flags, skb->fp_flow_info, skb->truesize, skb->head,
		skb->data, skb->tail, skb->end);

	if (skb->sk) {
		printk(KERN_ERR "sk family=%hu type=%u proto=%u\n",
			skb->sk->sk_family, skb->sk->sk_type,
			skb->sk->sk_protocol);
	}

	printk(KERN_ERR "skb->data dump:\n");
	for (i = 0; i < skb_headlen(skb); i++)
		printk("%02x ", skb->data[i]);
	printk("\n");

	printk(KERN_ERR "skb->mac.raw:\n");
...
/* Debug page NULL issue */
#ifdef CONFIG_OPENRG
				if (unlikely(!skb_shinfo(skb)->frags[i].page)) {
					dump_stack();
					skb_dump(skb);
					break;
				}
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb_shinfo(n)->nr_frags > MAX_SKB_FRAGS);
#endif
	return n;
}
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb_shinfo(old)->nr_frags > MAX_SKB_FRAGS);
#endif
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb_shinfo(new)->nr_frags > MAX_SKB_FRAGS);
#endif
}
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb2 && skb_shinfo(skb2)->nr_frags > MAX_SKB_FRAGS);
#endif
...
#ifdef CONFIG_OPENRG
	WARN_ON(result && skb_shinfo(result)->nr_frags > MAX_SKB_FRAGS);
#endif
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb_shinfo(newsk)->nr_frags > MAX_SKB_FRAGS);
#endif
}
...
#ifdef CONFIG_OPENRG
	WARN_ON(skb_shinfo(skb1)->nr_frags > MAX_SKB_FRAGS);
#endif
}

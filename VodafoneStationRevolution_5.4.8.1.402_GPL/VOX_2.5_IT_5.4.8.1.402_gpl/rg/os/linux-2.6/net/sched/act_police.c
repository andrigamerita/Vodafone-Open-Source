...
#define IPHDR_TOS(data) ((data)->tos)
#define IPHDR_CSUM(data) ((data)->check)

static void dscp_remark(struct sk_buff *skb, int new_dscp)
{
	struct iphdr *iphdr = skb->nh.iph;
	unsigned int l;

	/* -1 stands for "don't touch" */
	if (new_dscp == -1 || skb->protocol != htons(ETH_P_IP) || !iphdr)
		return;
	new_dscp |= (IPHDR_TOS(iphdr) & 3);

	/* Change DSCP and fix IP header checksum */
	l = ntohs(IPHDR_CSUM(iphdr)) + IPHDR_TOS(iphdr) - new_dscp;
	l = (l >> 16) + (l & 0xffff);
	l = l & 0xffff;
        IPHDR_CSUM(iphdr) = htons(l);
	IPHDR_TOS(iphdr) = new_dscp;
#ifdef CONFIG_NET_CLS_ACT
...
override:
	...
	police->over_dscpval= parm->over_dscpval;
	if (police->over_dscpval != -1)
	    police->over_dscpval <<= 2;
	police->under_dscpval= parm->under_dscpval;
	if (police->under_dscpval != -1)
	    police->under_dscpval <<= 2;
			  ...
			  struct tcf_result *res)
{
	...
	if (police->tcfp_ewma_rate &&
		...
		police->tcf_qstats.overlimits_bytes += skb->len;
		dscp_remark(skb, police->over_dscpval);
	...
	if (skb->len <= police->tcfp_mtu) {
		if (police->tcfp_R_tab == NULL) {
		        dscp_remark(skb, police->under_dscpval);
			spin_unlock(&police->tcf_lock);
...
tcf_act_police_dump(struct sk_buff *skb, struct tc_action *a, int bind, int ref)
{
	...
	opt.over_dscpval = police->over_dscpval>>2;
	opt.under_dscpval = police->under_dscpval>>2;
	if (police->tcfp_R_tab)

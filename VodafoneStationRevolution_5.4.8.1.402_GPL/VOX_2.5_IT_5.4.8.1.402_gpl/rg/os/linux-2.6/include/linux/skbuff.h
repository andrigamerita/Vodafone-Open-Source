...
#define SKB_DATA_FAP_RECYCLE      (1<<2)
...
#define IS_SKBSHINFO_DIRTYP_ACK(d)  (0xa0000000 == (0xf0000000 & (u32)(d)))
#define SET_SKBSHINFO_DIRTYP_ACK(d) do { \
              (d) = (u8 *) (SKBSHINFO_DIRTYP_ACK | (u32)(d));} while (0)
...
struct sk_buff {
    ...
    defined(CONFIG_RG_RGLOADER)
	__u16			tc_index;	/* traffic control index */
	__u16			phy_port_id;	/* HW switch phy port ID */
#ifdef CONFIG_NET_CLS_ACT
	__u16			tc_verd;	/* traffic control verdict */
#endif
#endif
#if defined(CONFIG_OPENRG) || defined(CONFIG_RG_RGLOADER)
	struct net_device	*bridge;	/* destination bridge device */
#endif
...
#define FREE_SKB		1
	...
	unsigned int    ti_selective_fwd_dev_info;
#endif
...
extern int kfree_skb_header(struct sk_buff *skb);
extern int __kfree_skb_header(struct sk_buff *skb);
...
static inline struct sk_buff *alloc_skb_header(unsigned int size, 
					...
					gfp_t priority)
{
	return __alloc_skb_header(size, data, priority, 0);
}
...
extern void	       __kclear_skb_state(struct sk_buff *skb);
...
static inline void kclear_skb_state(struct sk_buff *skb)
{
	if (atomic_read(&skb->users) == 1 || atomic_dec_and_test(&skb->users))
		__kclear_skb_state(skb);				
}

...
ifneq ($(CONFIG_RG_NETWORK_BOOT_IMAGE),y)
	...
	@$(foreach HW_HEADER,$(CONFIG_RG_HW_HEADERS),\
	  $(UPD_UTIL) -o -A -f $@ -h $(HW_HEADER) -d $(CONFIG_RG_DIST) \
	    -v `$(JMKE_BUILDDIR)/pkg/build/rg_version_info -c` $(EXT_VER) \
	    -t firmware;)
...
ifneq ($(CONFIG_RG_NETWORK_BOOT_IMAGE),y)
	...
	@$(foreach HW_HEADER,$(CONFIG_RG_HW_HEADERS),\
	  $(UPD_UTIL) -A -f $@ -h $(HW_HEADER) -d $(CONFIG_RG_DIST) \
	    -v `$(JMKE_BUILDDIR)/pkg/build/rg_version_info -c` $(EXT_VER) \
	    -k $(PRIV_KEY_FILE) -t firmware;)

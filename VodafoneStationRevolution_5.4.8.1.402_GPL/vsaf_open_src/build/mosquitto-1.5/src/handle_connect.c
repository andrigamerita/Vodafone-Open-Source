...
int handle__connect(struct mosquitto_db *db, struct mosquitto *context)
{
...
#ifdef WITH_TLS
	/* If TLS is used then set username based on client cert subject or CN if configured as such */
	...
	/* use username as client_id if configured as such */
	...
	/* if there is no username or client certificate present, and anonymous
	 * clients are forbidden then refuse the connect */
	if(!username_flag
	#ifdef WITH_TLS
	&& !context->listener->require_certificate
	#endif /* WITH_TLS */
	){

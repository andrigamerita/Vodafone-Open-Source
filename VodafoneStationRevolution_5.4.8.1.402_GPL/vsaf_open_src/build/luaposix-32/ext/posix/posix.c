...
static int Prealpath(lua_State *L)
{
	...
	if ((res = realpath(luaL_checkstring(L, 1), buf)) == NULL)
...
@return 0 on success, nil otherwise
@return error message if failed.
@usage posix.chroot("/mnt/new_root")
*/
static int Pchroot (lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);

	return pushresult(L, chroot(path), NULL);
}
...
static int Pmakedev(lua_State *L)
{
	int maj = luaL_checkint(L, 1);
	int min = luaL_checkint(L, 2);
	dev_t *dev = lua_newuserdata(L, sizeof(dev_t));
	    
	*dev = makedev(maj, min);
	return 1;
}

static int Pmknod(lua_State *L)
{
	const char *path = luaL_checkstring(L, 1);
	int mode = luaL_checkint(L, 2);
	dev_t *dev = (dev_t *)lua_touserdata(L, 3);
	return pushresult(L, mknod(path, mode, *dev), path);
}
...
static int Pmount (lua_State *L)
{
	const char *src = luaL_checkstring(L, 1);
	const char *dst = luaL_checkstring(L, 2);
	const char *fs = luaL_optstring(L, 3, NULL);
	unsigned int flags = luaL_optlong(L, 4, 0);
	const void *data = luaL_optstring(L, 5, NULL);

	return pushresult(L, mount(src, dst, fs, flags, data), NULL);
}

static int Pumount (lua_State *L)
{
	const char *dst = luaL_checkstring(L, 1);

	return pushresult(L, umount(dst), dst);
}

static int Pumount2 (lua_State *L)
{
	const char *dst = luaL_checkstring(L, 1);
	int flags = luaL_checkint(L, 2);

	return pushresult(L, umount2(dst, flags), dst);
}
...
#define O_DSYNC 0
	...
	MENTRY( MS_REMOUNT	);
	//MENTRY( MS_SILENT	);
	//MENTRY( MS_STRICTATIME	);
	MENTRY( MS_SYNCHRONOUS	);
